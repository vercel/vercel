...
def setup_logging(
    ...
    storage: contextvars.ContextVar[dict | None],
):
    ...
    class StreamWrapper:
        def __init__(
            self, stream: TextIO, stream_name: Literal["stdout", "stderr"]
        ):
            self.stream = stream
    ...
    logging.basicConfig(
        level=logging.INFO, handlers=[VCLogHandler()], force=True
    )
...
storage: contextvars.ContextVar[dict | None] = contextvars.ContextVar(
...
def enqueue_or_send_message(msg: dict):
    ...
    if _init_log_buf_bytes + enc_len <= _INIT_LOG_BUF_MAX_BYTES:
        ...
        with contextlib.suppress(Exception):
            ...
            decoded = base64.b64decode(payload.get("message", "")).decode(
                errors="ignore"
            )
...
try:
    __vc_spec = util.spec_from_file_location(
        _entrypoint_modname, _entrypoint_abs
    )
...
if "handler" in __vc_variables or "Handler" in __vc_variables:
    ...
    if not inspect.iscoroutinefunction(
        ...
        def vc_handler(event, context):
            ...
            if response.data:
                return_dict["body"] = base64.b64encode(response.data).decode(
                    "utf-8"
                )
                return_dict["encoding"] = "base64"
        ...
        class ASGICycle:
            ...
            def on_response(self):
                if self.body:
                    self.response["body"] = base64.b64encode(self.body).decode(
                        "utf-8"
                    )
