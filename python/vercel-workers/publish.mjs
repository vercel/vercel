/**
 * Publish vercel-workers on PyPI.
 *
...
const PYPROJECT_PATH = 'python/vercel-workers/pyproject.toml';
...
if (currentVersion === previousVersion && !force) {
  console.log(
    `Python vercel-workers version unchanged (${currentVersion}), skipping PyPI publication.`
  );
  process.exit(0);
}

console.log(
  `Python vercel-workers version changed: ${previousVersion || '(none)'} -> ${currentVersion}`
);
...
run('uv', [
  ...
  '-k',
  'not test_cqa_',
  'python/vercel-workers/tests',
]);

// Publish to PyPI (relies on OIDC trusted publishing env set up by the release.yml workflow)
run('uv', ['publish', '--directory', 'python/vercel-workers/']);

console.log(`Successfully published vercel-workers ${currentVersion} on PyPI.`);
