...
def _extract_timeout_seconds(result: Any, current: int | None) -> int | None:
...
async def _invoke_subscriptions_async(message: Any, metadata: MessageMetadata) -> int | None:
    """
    ...
    for sub in _select_subscriptions(topic):
        try:
            if inspect.iscoroutinefunction(sub.func):
                result = await sub.func(message, metadata)
            else:
                result = await asyncio.to_thread(sub.func, message, metadata)
        ...
        timeout_seconds = _extract_timeout_seconds(result, timeout_seconds)
...
async def handle_queue_callback_async(
    ...
    """
    Async callback handler used by ASGI wrappers.
    """
    ...
    try:
        ...
        if not is_v2beta:
            payload, delivery_count, created_at, receipt_handle = await asyncio.to_thread(
                callback.receive_message_by_id,
                queue_name,
                consumer_group,
                message_id,
                visibility_timeout_seconds=visibility_timeout_seconds,
        ...
        # Execute subscribers and ack/delay accordingly.
        timeout_seconds = await _invoke_subscriptions_async(payload, metadata)
        if receipt_handle:
            if timeout_seconds is not None:
                if extender is not None:
...
def get_asgi_app() -> ASGI:
    ...
    return build_asgi_app(handle_queue_callback_async)
