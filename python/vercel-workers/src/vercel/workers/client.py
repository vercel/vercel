...
def handle_queue_callback(
    ...
    Supports two CloudEvent modes:
    - v2beta binary: metadata in ``ce-*`` HTTP headers, payload in body.
    - v1beta structured: entire CloudEvent as JSON body (legacy fallback).
    ...
    try:
        ...
        if is_v2beta:
            # v2beta binary content mode: metadata in ce-* headers, payload in body.
            ...
            if not queue_name or not consumer_group or not message_id:
                return json_response(
                    400,
                    {"error": "missing required ce-vqs* headers"},
                )
            ...
            # Parse payload from body based on content type.
            ...
            # v1beta structured mode: parse CloudEvent JSON, then fetch message by ID.
        ...
        # Fail fast if no workers match this topic/consumer.
        if is_v2beta and not _select_subscriptions(queue_name, consumer_group):
            return json_response(
                500,
                {
                    "error": "no-matching-subscribers",
...
def send(
    ...
    *,
    ...
    # 202 Accepted means the message is deferred (regional failover) — no messageId yet.
    if response.status_code == 202:
        message_id = response.headers.get("Vqs-Message-Id") or data.get("messageId", "")
