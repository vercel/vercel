...
async def _handle_queue_callback_async(
    raw_body: bytes,
    environ: dict[str, Any] | None = None,
    subscriptions: list[_Subscription] = _subscriptions,
) -> tuple[int, list[tuple[str, str]], bytes]:
...
async def handle_queue_callback_async(
    ...
    return await _handle_queue_callback_async(raw_body, environ, _subscriptions)
...
def _build_asgi_app_for_subscriptions(subscriptions: list[_Subscription]) -> ASGI:
    async def handle(
        ...
        return await _handle_queue_callback_async(raw_body, environ, subscriptions)
