...
class TestWorkerBootstrapBridge(unittest.TestCase):
    def test_maybe_bootstrap_worker_service_app_skips_explicit_handler(self) -> None:
        module = types.SimpleNamespace(handler=object())

        with patch.object(vwr, "_resolve_worker_service_app") as resolve_worker_app:
            app = vwr.maybe_bootstrap_worker_service_app(module)

        self.assertIsNone(app)
        resolve_worker_app.assert_not_called()
