// Intentionally tests that "[" (unclosed class) is rejected.
...
wasm_tests! {
    ...
    fn find_iter_empty_pattern() {
        ...
        // Empty pattern matches between every character: "", "a", "", "b", ""
        assert_eq!(matches.len(), 3);
    ...
    #[test]
    fn dotall_flag_dot_matches_newline() {
        let re = Regex::new(r"(?s).+").unwrap();
        assert_eq!(re.replace_all("a\nb", "X"), "X");
    }

    // ---------------------------------------------------------------------------
    // \pX shorthand Unicode properties
    // ---------------------------------------------------------------------------

    #[test]
    fn unicode_property_shorthand_p_l() {
        let re = Regex::new(r"\pL+").unwrap();
        assert_eq!(re.replace_all("abc123", "[$0]"), "[abc]123");
    }

    #[test]
    fn unicode_property_shorthand_p_n() {
        let re = Regex::new(r"\pN+").unwrap();
        assert_eq!(re.replace_all("abc123", "[$0]"), "abc[123]");
    }

    #[test]
    fn unicode_property_negated_p_l() {
        let re = Regex::new(r"\PL+").unwrap();
        assert_eq!(re.replace_all("abc 123 def", "[$0]"), "abc[ 123 ]def");
    }

    // ---------------------------------------------------------------------------
    // Shorthands inside character classes (v flag)
    // ---------------------------------------------------------------------------

    #[test]
    fn char_class_with_word_shorthand() {
        let re = Regex::new(r"[\w]+").unwrap();
        // δ (Greek delta) is a word char in Unicode
        assert_eq!(re.replace_all("dδd!", "[$0]"), "[dδd]!");
    }

    #[test]
    fn char_class_with_digit_shorthand() {
        let re = Regex::new(r"[\d]+").unwrap();
        // Devanagari digits are Unicode digits
        assert_eq!(re.replace_all("1२३9", "[$0]"), "[1२३9]");
    }

    #[test]
    fn char_class_with_negated_word_shorthand() {
        let re = Regex::new(r"[\W]+").unwrap();
        assert_eq!(re.replace_all("hello world!", "[$0]"), "hello[ ]world[!]");
    }
    ...
    fn posix_class_negated() {
        let re = Regex::new(r"[[:^alpha:]]+").unwrap();
        assert_eq!(re.replace_all("abc123def", "[$0]"), "abc[123]def");
    }

    #[test]
    fn posix_class_combined() {
        let re = Regex::new(r"[[:alpha:][:digit:]]+").unwrap();
        assert_eq!(re.replace_all("abc123 def", "[$0]"), "[abc123] [def]");
    }
    ...
    #[test]
    fn char_class_with_pipe() {
        let re = Regex::new(r"[a|b]").unwrap();
        assert_eq!(re.replace_all("a|b+c", "X"), "XXX+c");
    }
    ...
    /// `\a` (bell, U+0007) must be translated to `\x07` for JS compatibility.
    #[test]
    fn bell_escape() {
        let re = Regex::new(r"\a").unwrap();
        assert_eq!(re.replace_all("x\x07y", "B"), "xBy");
    }

    /// `\a` inside a character class.
    #[test]
    fn bell_escape_in_class() {
        let re = Regex::new(r"[\a]").unwrap();
        assert_eq!(re.replace_all("x\x07y", "B"), "xBy");
    }

    // ---------------------------------------------------------------------------
    // \U (Unicode long hex) escape translation
    // ---------------------------------------------------------------------------

    /// `\U{...}` brace form must be translated to `\u{...}` for JS.
    #[test]
    fn unicode_long_hex_brace() {
        let re = Regex::new(r"\U{1F600}").unwrap();
        assert_eq!(re.replace_all("a\u{1F600}b", "X"), "aXb");
    }

    /// Fixed-width `\UXXXXXXXX` must also translate to `\u{...}`.
    #[test]
    fn unicode_long_hex_fixed() {
        let re = Regex::new(r"\U0001F600").unwrap();
        assert_eq!(re.replace_all("a\u{1F600}b", "X"), "aXb");
    }

    /// `\U{...}` with an ASCII code point.
    #[test]
    fn unicode_long_hex_ascii() {
        let re = Regex::new(r"\U{41}").unwrap();
        assert_eq!(re.replace_all("ABC", "X"), "XBC");
    }
    ...
    /// `(?U)` (swap-greed) is not supported by the JS backend.
    /// (Upstream regex supports it, so only assert on the stub.)
    #[test]
    fn swap_greed_flag_rejected() {
        if cfg!(not(feature = "upstream")) {
            assert!(Regex::new(r"(?U)a+").is_err());
        }
    }

    /// `(?x)` (ignore-whitespace) is not supported by the JS backend.
    /// (Upstream regex supports it, so only assert on the stub.)
    #[test]
    fn ignore_whitespace_flag_rejected() {
        if cfg!(not(feature = "upstream")) {
            assert!(Regex::new(r"(?x) a b").is_err());
        }
    }
}
