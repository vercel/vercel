...
def render_overview(section_sizes):
    ...
    iprint(f"{'Total (code+data):':<20} {BOLD}{format_bytes_long(total)}{RESET}")
    if total == 0:
        return
    ...
    iprint()
    iprint(f"{'Code:':<20} {format_bytes(total_code):>10}  {bar(code_frac, 30, CYAN)}  {code_frac*100:5.1f}%")
    iprint(f"{'Data:':<20} {format_bytes(total_data):>10}  {bar(data_frac, 30, MAGENTA)}  {data_frac*100:5.1f}%")
    iprint(f"  {DIM}{'.rodata:':<18}{RESET} {format_bytes(total_rodata):>10}")
    iprint(f"  {DIM}{'.data+.bss:':<18}{RESET} {format_bytes(total_data_seg):>10}")
...
def render_crate_table(title, crate_sizes, section_sizes, top_n):
    ...
    # Row layout (no leading indent):
    # "{i:>3}. {name:<name_w}  {total:>10}  {code:>10}  {data:>10}  {bar:20}  {pct:5.1f}%"
    # = 5 + name_w + 2+10 + 2+10 + 2+10 + 2+20 + 2+6 = name_w + 71
    row_w = name_w + 71
    iprint(f"     {'':>{name_w}}  {'total':>10}  {'code':>10}  {'data':>10}  {'':20}  {'%':>6}")
    iprint(f"{DIM}{'─' * row_w}{RESET}")
...
def render_items_table(entries, top_n):
    ...
    bar_w = max(10, _TERM_WIDTH - 28)  # leave room for size + pct
    ...
    for i, (section, crate, symbol, size) in enumerate(sorted_items[:top_n], 1):
        ...
        max_sym = _TERM_WIDTH - len(crate) - 14  # rank(4) + kind(4) + " :: "(4) + margin
        ...
        iprint(f"{DIM}{i:>3}.{RESET} {kind_tag}  {line1_label}")

        # Line 2: bar + size + percentage
        frac = size / max_size if max_size else 0
        pct = 100 * size / total_size if total_size else 0
        iprint(f"     {bar(frac, bar_w, c)}  {format_bytes(size):>10} {DIM}{pct:5.1f}%{RESET}")
...
def render_flamegraph_links(output_dir):
    ...
    for filename, desc, c in svgs:
        ...
        if p.exists():
            ...
            iprint(f"{c}{BAR_FULL}{RESET} {desc}")
            iprint(f"  {DIM}{link} ({sz}){RESET}")
...
""",
    ...
    parser.add_argument(
        ...
        help="indent all output by N spaces (e.g. 4 for commit messages)",
    )
