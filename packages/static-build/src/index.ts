...
export const build: BuildV2 = async ({
  ...
  if (useTanStackNitroFallback) {
    ...
    console.warn(
      'Detected TanStack Start with SSR enabled but no `nitro` dependency. Running `npx --yes -p nitro@npm:nitro-nightly@latest nitro build --builder vite` instead of the `vite build` script.'
    );
