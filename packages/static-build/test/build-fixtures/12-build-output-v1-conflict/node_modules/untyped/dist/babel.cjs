    ...
    visitor: {
        ...
        if (p.node.leadingComments) {
          ...
          if (p.node.value.type === "ObjectExpression") {
            ...
            p.node.value = t__namespace.objectExpression([
              t__namespace.objectProperty(t__namespace.identifier("$default"), p.node.value),
      ...
      FunctionDeclaration(p) {
        ...
        p.replaceWith(t__namespace.variableDeclaration("const", [
          t__namespace.variableDeclarator(t__namespace.identifier(p.node.id.name), astify({ $schema: schema }))
        ]));
      }
    }
  };
...
function astify(val) {
  ...
  return t__namespace.objectExpression(Object.getOwnPropertyNames(val).filter((key) => val[key] !== void 0 && val[key] !== null).map((key) => t__namespace.objectProperty(t__namespace.identifier(key), astify(val[key]))));
}
