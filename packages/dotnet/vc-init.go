// vc-init.go - Bootstrap wrapper for standalone .NET servers on Vercel
// Forked from packages/go/vc-init.go — adds ASPNETCORE_URLS for .NET port binding.
...
func main() {
	...
	cmd.Env = append(os.Environ(),
		fmt.Sprintf("PORT=%d", userPort),
		fmt.Sprintf("ASPNETCORE_URLS=http://0.0.0.0:%d", userPort),
	)
