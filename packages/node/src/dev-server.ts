    ...
    return `
      ...
      addEventListener('fetch', async (event) => {
        try {
          ...
          if (!edgeHandler) {
            throw new Error('No default export was found. Add a default export to handle requests. Learn more: https://vercel.com/docs/concepts/functions');
          }
  ...
  if (runtime && !validRuntimes.includes(runtime)) {
    throw new Error(
      `Invalid function runtime "${runtime}" for "${entrypoint}". Valid runtimes are: ${JSON.stringify(
        validRuntimes
      )}. Learn more: https://vercel.com/docs/concepts/functions`
    );
  }
