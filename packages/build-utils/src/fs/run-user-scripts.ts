...
export function getEnvForPackageManager({
  ...
  if (newPath && !alreadyInPath(newPath)) {
    ...
    if (detectedLockfile && detectedPackageManager) {
      ...
      const pnpmModernUsingPackageJsonPackageManager =
        (detectedPackageManager === 'pnpm@10.x' ||
          detectedPackageManager === 'pnpm@11.x') &&
        packageJsonPackageManager;

      if (pnpmModernUsingPackageJsonPackageManager) {
        const versionString =
...
export const PNPM_11_PREFERRED_AT = new Date('2026-04-28T09:34:08Z');
...
function detectPnpmVersion(
  ...
  switch (true) {
    ...
    case lockfileVersion === 9.0: {
      const projectPrefersPnpm11 =
        projectCreatedAt && projectCreatedAt >= PNPM_11_PREFERRED_AT.getTime();
      if (projectPrefersPnpm11) {
        return 'pnpm 11';
      }
...
export function getPathOverrideForPackageManager({
  ...
  if (cliType === 'pnpm' && packageJsonEngines?.pnpm) {
    // pnpm 10 and 11 are special because
    // https://pnpm.io/npmrc#manage-package-manager-versions
    const detectedRange = detectedPackageManger?.pnpmVersionRange;
    const usingDetected =
      (detectedRange !== '10.x' && detectedRange !== '11.x') ||
      !corepackPackageManager;
    if (usingDetected) {
