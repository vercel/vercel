  ...
  if (newPath && !alreadyInPath(newPath)) {
    ...
    if (detectedLockfile && detectedPackageManager) {
      ...
      if (pnpm10UsingPackageJsonPackageManager) {
        console.log(
          `Detected pnpm@10.x with package.json#packageManager ${packageJsonPackageManager}`
        );
      } else if (detectedV9PnpmLockfile) {
        const otherVersion =
          detectedPackageManager === 'pnpm@10.x' ? `pnpm@9.x` : `pnpm@10.x`;
        console.log(
          `Detected \`${detectedLockfile}\` ${lockfileVersion} which may be generated by pnpm@9.x or pnpm@10.x\nUsing ${detectedPackageManager} based on project creation date. To use ${otherVersion}, manually opt in using corepack (https://vercel.com/docs/deployments/configure-a-build#corepack).`
        );
      } else {
        const versionString =
