...
describe('Test `getEnvForPackageManager()`', () => {
  ...
  describe('with "pnpm"', () => {
    test.each<{
      ...
      },
      {
        ...
        consoleLogOutput:
          'Detected `pnpm-lock.yaml` 9 which may be generated by pnpm@9.x, pnpm@10.x, or pnpm@11.x\nUsing pnpm@10.x based on project creation date\nTo use a different version, set package.json#packageManager or package.json#devEngines.packageManager',
        consoleWarnOutput: null,
      },
      {
        name: 'should detect pnpm 9 from packageManager field even when created-at prefers 10',
        args: {
          cliType: 'pnpm',
          nodeVersion: getNodeVersionByMajor(18),
        ...
        consoleLogOutput:
          'Detected `pnpm-lock.yaml` version 9 generated by pnpm@9.x from package.json#packageManager pnpm@9.5.0',
        consoleWarnOutput: null,
      },
      {
        name: 'should detect pnpm 11 from devEngines.packageManager',
        args: {
          cliType: 'pnpm',
        ...
        want: {
          FOO: 'bar',
          PATH: `/pnpm11/node_modules/.bin${delimiter}foo`,
        },
        consoleLogOutput:
          'Detected `pnpm-lock.yaml` version 9 generated by pnpm@11.x from package.json#devEngines.packageManager',
        consoleWarnOutput: null,
      },
      {
    ...
    test('should detect pnpm 12 from packageManager when /pnpm12 exists', () => {
      mockPnpmMajorAvailable({ 11: true, 12: true });
      expect(
        getEnvForPackageManager({
          cliType: 'pnpm',
          nodeVersion: getNodeVersionByMajor(22),
          packageJsonPackageManager: 'pnpm@12.0.0',
          lockfileVersion: 9.0,
          env: {
            FOO: 'bar',
            PATH: 'foo',
        ...
        FOO: 'bar',
        PATH: `/pnpm12/node_modules/.bin${delimiter}foo`,
      });
      expect(consoleLogSpy).toHaveBeenCalledWith(
        'Detected `pnpm-lock.yaml` version 9 generated by pnpm@12.x from package.json#packageManager pnpm@12.0.0'
      );
    });
  });
  ...
  describe('with "bun"', () => {
    test.each<{
      ...
      {
        name: 'should set path for an explicit Bun 1.4 runtime',
        args: {
          cliType: 'bun',
          nodeVersion: getSupportedBunVersion('1.4.x'),
          packageJsonPackageManager: undefined,
          lockfileVersion: 1,
          env: {
            FOO: 'bar',
        ...
        want: {
          FOO: 'bar',
          PATH: `/bun1.4${delimiter}/bun1${delimiter}/usr/local/bin`,
        },
        consoleLogOutput: 'Detected `bun.lock` generated by bun@1.4.x',
      },
      {
        name: 'should set the Bun runtime path when using another package manager',
        args: {
          cliType: 'npm',
          nodeVersion: getSupportedBunVersion('1.4.x'),
          packageJsonPackageManager: undefined,
      ...
      {
        name: 'should preserve a previously selected Bun runtime path',
        args: {
          cliType: 'bun',
          ...
          env: {
            FOO: 'bar',
            PATH: `/bun1.4${delimiter}/usr/local/bin`,
          },
        },
        want: {
          FOO: 'bar',
      ...
      {
        name: 'should upgrade to Bun 1.4 for a `bun.lock` lockfileVersion 2, even with the default (unversioned) Bun runtime',
        args: {
          cliType: 'bun',
          nodeVersion: getSupportedBunVersion('1.x'),
          packageJsonPackageManager: undefined,
          lockfileVersion: 2,
          env: {
            FOO: 'bar',
      ...
      {
        name: 'should upgrade to Bun 1.4 for a `bun.lock` lockfileVersion 2 when no nodeVersion was resolved',
        args: {
          cliType: 'bun',
          nodeVersion: undefined,
          packageJsonPackageManager: undefined,
