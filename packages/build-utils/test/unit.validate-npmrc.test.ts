...
describe('validateNpmrc', () => {
  it.skip('should not error with no use-node-version', async () => {
    await expect(validateNpmrc(fixture('good'))).resolves.toBe(undefined);
  });

  it.skip('should throw when use-node-version is found', async () => {
    await expect(
      validateNpmrc(fixture('has-use-node-version'))
    ).rejects.toThrow('Detected unsupported');
  });

  it.skip('should not error when use-node-version is commented out', async () => {
    await expect(
