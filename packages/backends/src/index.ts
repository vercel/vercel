...
export const build: BuildV2 = async args => {
  ...
  return buildSpan.trace(async () => {
    // Run the user's build command before detecting the entrypoint, matching
    // the wrapper builders (`@vercel/express` et al.), where `@vercel/node`
    // invokes entrypoint resolution after the install and build steps. This
    // way entrypoints generated by the build (e.g. `package.json#main`
