...
import { getShipSessionDir } from './session-dir';
 ...
 *
 * A no-op outside a ship session, and never throws inside one: journaling is
 * an observer, and an observer that can fail the operation it observes is
 * worse than no observer. Writes are single `appendFileSync` calls well under
 * `PIPE_BUF`, so parallel tool calls interleave whole lines.
 */
export function recordSessionEvent(event: LedgerEvent): void {
  const dir = getShipSessionDir();
  if (!dir) return;
