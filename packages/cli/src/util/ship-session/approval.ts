...
/**
 * Ask the supervising ship process for the user's decision, from inside the
 * gated CLI invocation.
 *
 * The handshake is two files: this side writes `<id>.request.json` and polls
 * for `<id>.response.json`; ship watches the directory, prompts the human, and
 * writes the response. Both writes go through a temp file and a rename so the
 * reader never sees half a JSON document. No answer within the timeout is a
...
export async function requestApproval(
  ...
  try {
    ...
    // Leave nothing behind either way; ship also cleans the directory up.
...
/**
 * Ship's side of the handshake: polls the approvals directory, hands each
