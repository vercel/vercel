...
const GENERIC_KEY_PREFIX_PATTERN = /^KEYS?[_-]([A-Z0-9]+)(?:[_-]|$)/i;
...
function hasGenericKeyPrefix(key: string): boolean {
  const followingToken =
    GENERIC_KEY_PREFIX_PATTERN.exec(key)?.[1]?.toUpperCase();
  return Boolean(
