/**
 * Session plumbing for `vercel onboard`.
 *
 * When onboard drives a coding agent, every `vercel` invocation the agent
 * makes inherits `VERCEL_ONBOARD_SESSION_DIR`. That one variable powers two
 * mechanisms, both deliberately built from plain files so they can be
 * inspected with `cat` and tested with `fs`:
 *
 * - **The approval gate** (`approvals/`): commands that spend money, touch
 *   production, or delete remote resources pause and ask the supervising
 *   onboard process for the user's decision before executing. The gate is
 *   called from inside each command handler after its own argument parsing —
 *   never from shell-command classification — so it is deterministic on what
 *   is actually about to happen, and does not depend on the agent complying
 *   with prose.
 *
 ...
 */
export { ONBOARD_SESSION_DIR_ENV, getOnboardSessionDir } from './session-dir';
