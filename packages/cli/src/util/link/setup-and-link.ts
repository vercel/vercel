...
export async function setupRequiredIntegrations(
  client: Client,
  {
    cwd,
    autoConfirm,
  ...
  const localConfig = await readConfig(cwd);
  if (localConfig instanceof Error) {
    output.debug(
      `Skipping integration requirements. Failed to read config: ${localConfig.message}`
    );
    return;
  }
