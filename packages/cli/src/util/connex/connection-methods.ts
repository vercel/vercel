...
export interface ConnexServiceInfo {
  ...
  /**
   * Slug of the `connectionMethods[]` entry the api resolved,
   * when the caller requested a particular url or a connection method.
   */
  resolvedConnectionMethod?: string;
}
...
export async function resolveConnexConnectionMethod(
  ...
  // Pick the connection method that the api already resolved.
  // A conflicting --target falls back to the choosers.
  if (opts.resolvedConnectionMethod) {
    const method = methods.find(
      candidate => candidate.connectionMethod === opts.resolvedConnectionMethod
    );
    if (method && (!explicitTarget || servesTarget(method, explicitTarget))) {
      if (opts.interactive && !opts.skipConfirm) {
        await confirmMethodSelection(client, opts.serviceName, method);
      }
      return { method, target: explicitTarget ?? soleTargetOf(method) };
    }
  }
...
/** Shows what is about to be created when the user didn't pick it. */
async function confirmMethodSelection(
  ...
  const confirmed = await client.input.confirm(
    `Connect to ${serviceName} with ${method.label}${suffix}?`,
    true
  );
  if (!confirmed) {
