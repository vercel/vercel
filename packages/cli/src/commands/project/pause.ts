...
export default async function pause(
  ...
  if (!canPrompt(client)) {
    ...
    const interactiveCommand = getCommandNamePlain(
      `project pause ${args[0] ? `${args[0]} ` : ''}${globalFlags.join(' ')}`.trim()
    );
    outputActionRequired(
      client,
      {
        ...
        message:
          'Pausing a project stops serving production traffic and visitors will see an error page. ' +
          'This cannot be confirmed non-interactively: the user must run this command in a terminal and type the project name to confirm.',
  ...
  output.print(
    prependEmoji(
      `Pausing ${chalk.bold(project.name)} will stop serving production traffic and visitors will see an error page.\n`,
      emoji('warning')
    )
  );
  const entered = await client.input.text({
    message: `Type ${project.name} to confirm pausing production traffic:`,
  });
  if (entered !== project.name) {
    output.log('Canceled');
    return 0;
  }
