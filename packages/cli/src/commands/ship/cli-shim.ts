...
/**
 * Make the agent's `vercel` be the same CLI that is running `vercel ship`.
 *
 ...
 * release routinely shadows the copy running ship (observed on the first real
 * session after the gate shipped — the env variable propagated perfectly and
 * the session directory stayed empty, because every command ran through the
 * old global binary).
 *
 * So ship writes two shims, `vercel` and `vc`, that exec this process's own
...
export async function installCliShim(
  ...
  try {
    ...
    output.debug(
      `ship: could not install the CLI shim: ${
