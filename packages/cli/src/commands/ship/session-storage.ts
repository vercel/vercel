...
import { LEDGER_FILENAME } from '../../util/ship-session';

/**
 * Where a ship session keeps its working data — and afterwards, its record.
 *
...
/**
 * Create the session directory, preferring `.agent-runs/ship/<iso>-<pid>` in
...
export async function createSessionDir(
  ...
  try {
    ...
    await mkdir(join(runsDir, SHIP_DIRNAME), { recursive: true });
    ...
    const dir = join(runsDir, SHIP_DIRNAME, name);
    await mkdir(dir);
    return { dir, persistent: true };
  } catch (error) {
    output.debug(
      `ship: falling back to a temp session dir: ${
        error instanceof Error ? error.message : String(error)
      }`
    );
    return {
      dir: await mkdtemp(join(tmpdir(), 'vercel-ship-')),
      persistent: false,
    };
  }
}
...
async function pruneSessions(shipDir: string): Promise<void> {
  try {
    const entries = (await readdir(shipDir)).sort();
    const stale = entries.slice(0, Math.max(0, entries.length - KEEP_SESSIONS));
    for (const entry of stale) {
      await rm(join(shipDir, entry), { recursive: true, force: true });
    }
  } catch {
