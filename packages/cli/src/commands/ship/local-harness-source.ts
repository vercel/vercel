...
/**
 * Points `vercel ship` at a local checkout of the `vercel/ai` monorepo instead
 * of the npm registry.
 *
 ...
 */
export const HARNESS_SOURCE_ENV_VAR = 'VERCEL_SHIP_HARNESS_SOURCE';
...
export async function packLocalHarnessPackages(options: {
  ...
  if (cached) {
    output.debug('ship: local harness build unchanged, reusing the tarballs');
    return cached;
  }
...
async function writeCachedPack(
  ...
  try {
    ...
    output.debug(
      `ship: could not record the local harness fingerprint: ${
