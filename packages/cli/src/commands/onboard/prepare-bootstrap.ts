  ...
  try {
    if (await pathExists(configPath)) {
      ...
      if (existing.includes(PNPM_PLACEHOLDER)) {
        ...
        output.debug(
          `onboard: replaced the pnpm build-approval placeholder in ${configPath}`
        );
        return;
      }

      output.debug(`onboard: ${configPath} already present, leaving it alone`);
      return;
    }

    await outputFile(configPath, ALLOW_BUILDS, 'utf-8');
    output.debug(
      `onboard: pre-authorized harness bridge builds in ${configPath}`
    );
  } catch (err) {
    // Never block a session on this. If it fails, the adapter's own bootstrap
    // error is the more useful thing for the user to see.
    output.debug(
      `onboard: could not prepare the harness bootstrap directory: ${
...
async function repairIncompleteBootstrap(bootstrapDir: string): Promise<void> {
  ...
  try {
    ...
    output.debug(
      `onboard: the installed bridge is not runnable (${
