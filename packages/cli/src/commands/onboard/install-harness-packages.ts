...
async function confirmInstall(options: {
  ...
  output.log(
    `${harnessLabel} needs ${missing.join(', ')} — installed into ${dir}, ` +
...
async function runNpmInstall(options: {
  ...
  try {
    ...
    // `VERCEL_ONBOARD_HARNESS_SOURCE` exists for. Overriding the setting
 ...
 *
 * Runs without a confirmation prompt: setting `VERCEL_ONBOARD_HARNESS_SOURCE` is
...
async function createManagedLoader(
  ...
  await writeFile(
    loaderPath,
    [
      '// Generated by `vercel onboard`. Safe to delete.',
  ...
  output.debug(`onboard: harness runtime loaded from ${dir} (${origin})`);
