...
/**
 * Print how to carry on with the agent directly, without `vercel onboard`.
 *
...
export async function printContinuation(options: {
  ...
  const resume =
    sessionId && commands.byId ? commands.byId(sessionId) : commands.latest;

  // Quoted because the path is copy-pasted, and config directories on macOS
  // contain spaces.
  output.print('\n');
  output.log(
    `Continue in ${options.harnessLabel}: ${chalk.cyan(
      `cd ${JSON.stringify(options.workspace)} && ${resume}`
    )}\n`
  );
}
...
export async function resolveSessionId(options: {
  ...
  try {
    ...
    output.debug(
      `onboard: could not resolve the agent session id: ${
 ...
 * persists that turn asynchronously while onboard is already heading into the
