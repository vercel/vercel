...
import { COMMANDS } from '@vercel-internals/constants';
import { help } from '@vercel-internals/get-help-output';
...
export default async (client: Client): Promise<number> => {
  ...
  const deployCommand = COMMANDS.find(command => command.name === 'deploy')!;
  const argOptions: {
    [k in string]:
      | BooleanConstructor
      | StringConstructor
