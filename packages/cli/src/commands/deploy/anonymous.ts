...
export async function runImplicitBuild(
  ...
  const projectJsonExists = await fs.pathExists(projectJsonPath);
  ...
  // Temporary deployments are first deployments, but their local builds do not inherit this API-provided flag.
  ...
  try {
    ...
    if (previousVercelFirstDeployment === undefined) {
