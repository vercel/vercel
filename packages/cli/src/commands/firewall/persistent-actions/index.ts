...
export default async function persistentActions(
  client: Client,
  args: string[],
  ...
  if (isInspect) {
    if (needHelp) {
      telemetry.trackCliFlagHelp('firewall', 'persistent-actions:inspect');
      output.print(
        help(persistentActionsInspectSubcommand, {
    ...
    telemetry.trackCliSubcommandPersistentActionsInspect('inspect');
