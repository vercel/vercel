...
export default async function publish(client: Client, argv: string[]) {
  ...
  if (!draft || draft.changes.length === 0) {
    output.warn(
      `No staged changes to publish. Make changes first with ${chalk.cyan(getCommandName('firewall rules add'))} or ${chalk.cyan(getCommandName('firewall ip-blocks block'))}.`
    );
    return 0;
  }
