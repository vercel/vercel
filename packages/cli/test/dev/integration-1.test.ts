...
test('[vercel dev] should handle config errors thrown in edge functions', async () => {
  ...
  try {
    ...
    expect(stderr).toContain(
      'api/edge-error-config.js: unsupported "runtime" value in `config`: "invalid-runtime-value" (must be one of: ["edge","experimental-edge","nodejs","bun"]). Learn more: https://vercel.link/creating-edge-functions'
    );
  } finally {
    await dev.kill();
  }
