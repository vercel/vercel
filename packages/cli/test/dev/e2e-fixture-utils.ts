...
export async function runFixtureAgainstDev(fixture: FixtureCase) {
  ...
  try {
    ...
    let probes = fixture.probes.filter(p => !p.skipDev);
    if (fixture.xfail !== null) {
      // Expected failures only need to fail once. Deployment-oriented probe
      // retries can otherwise make a known vc dev failure take minutes.
      probes = probes.map(probe => ({ ...probe, retries: 0, retryDelay: 0 }));
    }
...
export function registerFixtureDevTests(
  ...
  for (const f of fixtures) {
    if (f.xfail !== null) {
      ...
      it.fails(
        `[vc dev] e2e fixture ${f.name} (VC_DEV_XFAIL${reason})`,
        { retry: 0 },
