...
test('[vc redeploy] should redeploy an existing deployment', async () => {
  const projectDir = await setupE2EFixture('static-deployment');
  await vcLink(projectDir);

  let { exitCode, stdout, stderr } = await execCli(binaryPath, [], {
    cwd: projectDir,
  });
  ...
  ({ exitCode, stdout, stderr } = await execCli(
    binaryPath,
    ['redeploy', deploymentUrl],
    {
      cwd: projectDir,
    }
  ));
  ...
  expect(exitCode, formatOutput({ stdout, stderr })).toBe(0);
  expect(stderr).toMatch(/Building\nCompleting\nProduction:/);
});

test('[vc redeploy] should redeploy and not wait for completion', async () => {
  const projectDir = await setupE2EFixture('static-deployment');
  await vcLink(projectDir);
  ...
  ({ exitCode, stdout, stderr } = await execCli(
    binaryPath,
    ['redeploy', deploymentUrl, '--no-wait'],
    {
      cwd: projectDir,
    }
  ));
