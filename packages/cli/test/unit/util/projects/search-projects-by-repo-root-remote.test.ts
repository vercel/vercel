...
describe('searchProjectsByRepoRoot() remote selection', () => {
  ...
  it('uses origin without prompting when the repo has several remotes', async () => {
    ...
    const result = await searchProjectsByRepoRoot({
      client,
      cwd: repoDir,
      orgs: [org],
    });
    ...
    expect(result.remoteNames).toEqual(['aaa', 'origin', 'zzz']);
    expect(requestedRepoUrls).toEqual(['https://github.com/test/origin.git']);
  });

  it('searches a specific remote when remoteName is provided', async () => {
    useUser();
    const requestedRepoUrls: string[] = [];
    client.scenario.get('/v9/projects', (req, res) => {
      const url = new URL(req.url || '', 'http://localhost');
    ...
    expect(select).not.toHaveBeenCalled();
    expect(result.remote?.remoteName).toEqual('zzz');
    expect(requestedRepoUrls).toEqual(['https://github.com/test/zzz.git']);
  });
});
