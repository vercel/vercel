...
describe('getLinkedProject with team env vars', () => {
  ...
  it('reports the env var the user set when the pair is incomplete', async () => {
    const cwd = setupTmpDir('env-team-missing-team');
    ...
    expect(link).toEqual({ status: 'error', exitCode: 1 });
    expect(client.stderr.getFullOutput()).toContain(
      'You specified `VERCEL_PROJECT_ID` but you forgot to specify `VERCEL_TEAM`'
    );
  });

  it('falls back to the local `.vercel` link when only `VERCEL_TEAM` is set', async () => {
    const cwd = setupTmpDir('env-team-only-local-link');
    const vercelDir = join(cwd, '.vercel');
    await mkdirp(vercelDir);
    await writeJSON(join(vercelDir, 'project.json'), {
      orgId: 'team_dummy',
    ...
    const link = await getLinkedProject(client, { cwd });
    if (link.status !== 'linked') {
      throw new Error(`Expected to be linked, got "${link.status}"`);
    }
    expect(link.project.id).toEqual('local-project');
    expect(link.org.id).toEqual('team_dummy');
  });

  it('reports `not_linked` — not an error — when only `VERCEL_TEAM` is set and there is no local link', async () => {
    const cwd = setupTmpDir('env-team-only-no-local-link');
    ...
    const link = await getLinkedProject(client, { cwd });
    expect(link.status).toEqual('not_linked');
    expect(client.stderr.getFullOutput()).not.toContain('forgot to specify');
  });

  it('still requires the pair for the legacy `VERCEL_ORG_ID`', async () => {
    const cwd = setupTmpDir('env-org-id-only');
    const vercelDir = join(cwd, '.vercel');
    await mkdirp(vercelDir);
    await writeJSON(join(vercelDir, 'project.json'), {
      orgId: 'team_dummy',
  ...
  it('falls back to the local link when `VERCEL_TEAM` overrides `VERCEL_ORG_ID` without a project ID', async () => {
    const cwd = setupTmpDir('env-team-over-org-id-local-link');
    const vercelDir = join(cwd, '.vercel');
    await mkdirp(vercelDir);
    await writeJSON(join(vercelDir, 'project.json'), {
      orgId: 'team_dummy',
    ...
    // `VERCEL_TEAM` wins, so this is a scope-only selection and must not
    // resurrect the legacy `VERCEL_ORG_ID` pairing requirement.
    process.env.VERCEL_TEAM = team.slug;
    process.env.VERCEL_ORG_ID = 'team_ignored';
