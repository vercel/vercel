...
describe('createDetectEntrypoint', () => {
  beforeEach(() => {
    ...
    detectRust.mockResolvedValue({ kind: 'file', entrypoint: 'src/main.rs' });
  });
  ...
  it('routes the rust runtime framework to @vercel/rust via importBuilders', async () => {
    importBuilders.mockResolvedValue(mockBuilder('@vercel/rust', detectRust));
    const dispatch = createDetectEntrypoint('/abs/project');
    const result = await dispatch({
      workPath: 'services/svc',
      framework: 'rust',
    ...
    expect(detectRust).toHaveBeenCalledWith({
      workPath: '/abs/project/services/svc',
    });
    expect(detectGo).not.toHaveBeenCalled();
    expect(result).toEqual({ kind: 'file', entrypoint: 'src/main.rs' });
  });
