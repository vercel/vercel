...
describe('getTeamByIdOrSlug', () => {
  ...
  it('sends the current team as a query parameter by default', async () => {
    const team = createTeam('team_example', 'example-team', 'Example Team');
    ...
    expect(teamIdParam).toBe('team_other');
  });

  it('omits the current team when useCurrentTeam is false', async () => {
    const team = createTeam('team_example', 'example-team', 'Example Team');
    client.config.currentTeam = 'team_other';
    let teamIdParam: string | null = null;
    client.scenario.get('/teams/team_example', (req, res) => {
    ...
    await getTeamByIdOrSlug(client, team.id, { useCurrentTeam: false });

    expect(teamIdParam).toBeNull();
  });

  it('caches the team under its ID after a lookup by slug', async () => {
    const team = createTeam('team_example', 'example-team', 'Example Team');
    ...
    const cachedTeam = await getTeamByIdOrSlug(client, team.id);

    expect(cachedTeam).toEqual(team);
    expect(teamFetchCount).toBe(1);
  });

  it('caches the team under its slug after a lookup by ID', async () => {
    const team = createTeam('team_example', 'example-team', 'Example Team');
    let teamFetchCount = 0;
    client.scenario.get('/teams/team_example', (_req, res) => {
      teamFetchCount++;
    ...
    await getTeamByIdOrSlug(client, team.id);
    const cachedTeam = await getTeamByIdOrSlug(client, team.slug);
