...
} from '../../../../src/util/build/builder-process';
...
describe('build-runner', () => {
  ...
  describe('SubprocessBuildRunner', () => {
    ...
    it('forks with the per-build env when set', async () => {
      driveBuild({ result: { output: {} } });
      const env = { VERCEL_PROJECT_SETTINGS_BUILD_COMMAND: 'next build' };
      const runner = new SubprocessBuildRunner(makeContext({ env }));
    ...
    it('falls back to process.env when no per-build env is set', async () => {
      driveBuild({ result: { output: {} } });
      const runner = new SubprocessBuildRunner(makeContext());
    ...
    it('merges the meta the worker mutated back into the shared meta object', async () => {
      ...
      // Set, and scalar latches land on the shared object.
    ...
    it('re-prototypes outputs and preserves a Lambda shared across Prerenders', async () => {
      ...
      driveBuild({ result: { output }, outputVersion: 2 });
      const runner = new SubprocessBuildRunner(makeContext());
    ...
    it('re-prototypes the single output of a v3 result', async () => {
      driveBuild({
        result: { output: { type: 'Lambda', files: {}, handler: 'index.js' } },
        outputVersion: 3,
      });
      const runner = new SubprocessBuildRunner(makeContext());

      const built = (await runner.build()) as unknown as { output: unknown };

      expect(built.output).toBeInstanceOf(Lambda);
    });

    it('re-prototypes a v2 output map containing a key named "type"', async () => {
      // A v2 output map is keyed by output path, so `type` is a legal key. Deciding v2-vs-v3
      // by looking for `output.type` would read the map itself as a single v3 output and
      // silently leave every entry un-prototyped, so the version comes from the worker.
      driveBuild({
        result: {
          output: {
            type: { type: 'FileBlob', data: 'plain' },
            index: { type: 'Lambda', files: {}, handler: 'index.js' },
          },
        },
        outputVersion: 2,
      });
      ...
      expect(built.output.index).toBeInstanceOf(Lambda);
      expect(built.output.type).toBeInstanceOf(FileBlob);
    });
...
describe('mergeWorkerMeta()', () => {
  ...
  it('last-write-wins when a key changes type (Set replaced by a non-Set, latch replaced by an object)', () => {
    const setTarget: Record<string, unknown> = {
      runNpmInstallSet: new Set(['/a']),
    };
    mergeWorkerMeta(setTarget, { runNpmInstallSet: '/b' as never });
    expect(setTarget.runNpmInstallSet).toBe('/b');

    const latchTarget: Record<string, unknown> = { someFlag: true };
    const replacement = { nested: 1 };
    mergeWorkerMeta(latchTarget, { someFlag: replacement as never });
    expect(latchTarget.someFlag).toBe(replacement);
  });
});
