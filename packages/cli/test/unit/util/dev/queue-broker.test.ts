...
import {
  QueueBroker,
  topicPatternToRegex,
  type QueueConsumerRegistration,
...
): QueueConsumerRegistration {
  return {
    name,
    topics: topics.map(topic => ({ topic })),
    callbackUrl: () => {
      const origin = getServiceOrigin(name);
...
function makeWebService(name: string): QueueConsumerRegistration {
  return {
    name,
    topics: [],
    callbackUrl: () => null,
...
describe('QueueBroker', () => {
  ...
  describe('enqueue', () => {
    it('returns unique messageIds', () => {
      broker = new QueueBroker([makeWorkerService('worker-a', ['orders'])]);
    ...
    it('ignores non-worker services', async () => {
      broker = new QueueBroker([makeWebService('frontend')]);
