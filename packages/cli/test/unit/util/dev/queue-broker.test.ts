...
function lastDispatchRequest(): {
  headers: Record<string, string>;
  body: Buffer;
} {
  const call = mockFetch.mock.calls[mockFetch.mock.calls.length - 1];
  return {
    headers: (call[1] as any).headers,
    body: Buffer.from((call[1] as any).body),
  };
}
...
describe('QueueBroker', () => {
  ...
  describe('enqueue', () => {
    ...
    it('dispatches v2beta binary callback to matching worker', async () => {
      broker = new QueueBroker(
        [makeWorkerService('worker-a', ['orders'])],
        getServiceOrigin
      );
      ...
      const { headers, body } = lastDispatchRequest();
    ...
    it('dispatches to worker subscribed to multiple topics via topics[]', async () => {
      ...
      const headers = mockFetch.mock.calls.map(c => (c[1] as any).headers);
      expect(headers[0]['ce-vqsqueuename']).toBe('orders');
      expect(headers[1]['ce-vqsqueuename']).toBe('events');
    });
  ...
  describe('receiveMessages', () => {
    it('leases visible messages for the requested queue and consumer', async () => {
      broker = new QueueBroker(
        [makeWorkerService('worker-a', ['orders'])],
        getServiceOrigin
      );
      ...
      const results = broker.receiveMessages('orders', 'worker-a', {
        limit: 2,
        visibilityTimeoutSeconds: 45,
      });

      expect(results).toHaveLength(2);
      expect(results[0].receiptHandle).toBeTruthy();
      expect(results[1].receiptHandle).toBeTruthy();
      expect(results[0].deliveryCount).toBe(1);
      expect(results[1].deliveryCount).toBe(1);
    });

    it('does not lease messages from other queues', () => {
      ...
      expect(
        broker.receiveMessages('orders', 'worker-a', {
  ...
  describe('receiveMessageById', () => {
    it('returns a leased message when it is available', async () => {
      broker = new QueueBroker(
        [makeWorkerService('worker-a', ['orders'])],
        getServiceOrigin
      );
      ...
      const result = broker.receiveMessageById(
        'orders',
        'worker-a',
        messageId,
        30
      ...
      if (result.status === 'ok') {
        ...
        expect(result.message.receiptHandle).toBeTruthy();
      }
    });

    it('returns not_available for delayed messages', () => {
      broker = new QueueBroker(
        [makeWorkerService('worker-a', ['orders'])],
        getServiceOrigin
      );
      ...
      expect(
        broker.receiveMessageById('orders', 'worker-a', messageId, 30)
  ...
  describe('acknowledge', () => {
    ...
    it('returns false for unknown consumer group', () => {
      ...
      expect(broker.acknowledge('any', 'unknown-group', 'receipt')).toBe(false);
    });
    ...
    it('cleans up message when acked in all groups', async () => {
      ...
      const receiptHandle = broker.receiveById(
        messageId,
        'worker-a'
      )!.receiptHandle;
      broker.acknowledge(messageId, 'worker-a', receiptHandle);
    ...
    it('keeps message receivable in other groups after partial ACK', async () => {
      ...
      // ACK in group A
      const receiptHandleA = broker.receiveById(
        messageId,
        'worker-a'
      )!.receiptHandle;
      broker.acknowledge(messageId, 'worker-a', receiptHandleA);
  ...
  describe('changeVisibility', () => {
    ...
    it('prevents lease expiry when visibility is extended', async () => {
      ...
      broker.changeVisibility(messageId, 'worker-a', receiptHandle, 300);
      ...
      expect(broker.acknowledge(messageId, 'worker-a', receiptHandle)).toBe(
        true
      );
    });
  });
