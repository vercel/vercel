...
import { ApprovalWatcher } from '../../../../src/util/onboard-session/approval';
import { confirmGatedOperation } from '../../../../src/util/onboard-session/gate';
import { readLedger } from '../../../../src/util/onboard-session/ledger';
import { ONBOARD_SESSION_DIR_ENV } from '../../../../src/util/onboard-session/session-dir';
...
describe('confirmGatedOperation', () => {
  ...
  beforeEach(async () => {
    sessionDir = await mkdtemp(join(tmpdir(), 'onboard-gate-test-'));
    previousEnv = process.env[ONBOARD_SESSION_DIR_ENV];
  ...
  afterEach(async () => {
    if (previousEnv === undefined) {
      ...
      process.env[ONBOARD_SESSION_DIR_ENV] = previousEnv;
    }
    await rm(sessionDir, { recursive: true, force: true });
  });

  it('is a no-op outside an onboard session', async () => {
