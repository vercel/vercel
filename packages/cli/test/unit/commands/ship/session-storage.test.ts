...
import {
  createSessionDir,
  finalizeSessionDir,
} from '../../../../src/commands/ship/session-storage';

describe('ship session storage', () => {
  let workspace: string;

  beforeEach(async () => {
    workspace = await mkdtemp(join(tmpdir(), 'ship-storage-test-'));
  });
  ...
  it('creates the session dir next to the harness run data, gitignored', async () => {
    ...
    expect(storage.dir).toContain(join(workspace, '.agent-runs', 'ship'));
    expect(
  ...
  it('leaves nothing behind when the session recorded nothing', async () => {
    ...
    const sessions = await readdir(join(workspace, '.agent-runs', 'ship'));
    expect(sessions).toEqual([]);
  });

  it('prunes old session records, keeping the newest', async () => {
    const shipDir = join(workspace, '.agent-runs', 'ship');
    await mkdir(shipDir, { recursive: true });
    // ISO-shaped names sort chronologically.
    for (let i = 10; i < 33; i++) {
      await mkdir(join(shipDir, `2026-01-${i}T00-00-00-000Z-1`));
    }
    ...
    const sessions = (await readdir(shipDir)).sort();
