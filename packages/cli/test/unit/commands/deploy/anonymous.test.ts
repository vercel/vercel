...
function mockDeploymentEndpoints({ checksFailed = false } = {}) {
  ...
  client.scenario.post('/v13/deployments', (req, res) => {
    requests.push({
      ...
      projectSettings: req.body.projectSettings,
    });
    res.json({
      id: 'dpl_anon',
...
describe('deploy [anonymous]', () => {
  ...
  it('deploys temporarily without prompting', async () => {
    ...
    if (process.platform !== 'win32') {
      expect((await fs.stat(join(cwd, ANONYMOUS_FILE))).mode & 0o777).toEqual(
        0o600
      );
    }
  ...
  it('rebuilds stale output and persists the detected framework', async () => {
    const cwd = setupUnitFixture('commands/deploy/anonymous');
    await fs.writeFile(join(cwd, 'server.ts'), 'export default {}');
    client.cwd = cwd;
    client.authConfig = {};
    mockBootstrap({
      token: 'vcn_test',
    ...
    buildMock.mockImplementation(async () => {
      expect(process.env.VERCEL_FIRST_DEPLOYMENT).toEqual('1');
      ...
      await fs.outputJSON(join(output, 'builds.json'), {
        ...
        detectedFramework: { status: 'detected', slug: 'node' },
      });
      await fs.outputJSON(join(output, 'config.json'), {
        version: 3,
        routes: [],
      });
      await fs.outputFile(join(output, 'functions/server.func/index.js'), '');
      return 0;
    });

    const originalFirstDeployment = process.env.VERCEL_FIRST_DEPLOYMENT;
    client.setArgv('deploy', '--temporary');
    const exitCode = await deploy(client);
    ...
    expect(requests[0].projectSettings).toMatchObject({ framework: 'node' });
    expect(process.env.VERCEL_FIRST_DEPLOYMENT).toEqual(
      originalFirstDeployment
    );
    expect(await fs.pathExists(join(cwd, '.vercel/project.json'))).toEqual(
      false
    );
  ...
  it('reuses output only with explicit --prebuilt', async () => {
    const cwd = setupUnitFixture('commands/deploy/anonymous');
    client.cwd = cwd;
    client.authConfig = {};
    mockBootstrap({ token: 'vcn_test', expiresAt: Date.now() + 3600_000 });
    mockDeploymentEndpoints();

    client.setArgv('deploy', '--temporary', '--prebuilt');
    const exitCode = await deploy(client);
  ...
  it('preserves an explicit framework opt-out', async () => {
    const cwd = setupUnitFixture('commands/deploy/anonymous');
    await fs.outputJSON(join(cwd, 'vercel.json'), { framework: null });
    client.localConfig = undefined;
    ...
    expect(exitCode).toEqual(0);
    expect(requests[0].projectSettings).toMatchObject({ framework: null });
  });
  ...
  it('fails the deploy when the implicit build fails', async () => {
    ...
    const originalFirstDeployment = process.env.VERCEL_FIRST_DEPLOYMENT;
    process.env.VERCEL_FIRST_DEPLOYMENT = 'existing';

    let exitCode: number;
    let restoredFirstDeployment: string | undefined;
    try {
      client.setArgv('deploy', '--temporary');
      exitCode = await deploy(client);
      restoredFirstDeployment = process.env.VERCEL_FIRST_DEPLOYMENT;
    } finally {
      if (originalFirstDeployment === undefined) {
        delete process.env.VERCEL_FIRST_DEPLOYMENT;
      } else {
        process.env.VERCEL_FIRST_DEPLOYMENT = originalFirstDeployment;
      }
    }

    expect(exitCode).toEqual(1);
    expect(requests.length).toEqual(0);
    expect(restoredFirstDeployment).toEqual('existing');
  });
});
