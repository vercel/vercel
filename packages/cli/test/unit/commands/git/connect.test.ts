...
describe('git connect', () => {
  ...
  describe('connecting an unlinked project', () => {
    ...
    beforeEach(() => {
      cwd = setupTmpDir();
      initGitRepo(cwd, { origin: 'https://github.com/user/repo.git' });
      // Note: "unlinked" here means no git connection, but the project IS linked to Vercel
      createProjectLink(cwd, 'unlinked');
      useUser();
      useTeams('team_dummy');
      useProject({
        ...defaultProject,
    ...
    describe('passing [git url]', () => {
      it('tracks telemetry', async () => {
        ...
        // When passing a different URL than the local remote, it prompts for confirmation
    ...
    it('connects an unlinked project', async () => {
      ...
      // With a valid project link, it connects directly using the local git remote
      await expect(client.stderr).toOutput(
        `Connecting GitHub repository: https://github.com/user/repo`
      );
  ...
  it('should fail when there is no git config', async () => {
    const cwd = setupTmpDir();
    // No git repo initialized - just a plain directory
    createProjectLink(cwd, 'no-git-config');
    client.cwd = cwd;
    useUser();
    useTeams('team_dummy');
    useProject({
  ...
  it('should fail when there is no remote url', async () => {
    const cwd = setupTmpDir();
    initGitRepo(cwd, {}); // Git repo with no remotes
    createProjectLink(cwd, 'no-remote-url');
    client.cwd = cwd;
    useUser();
    useTeams('team_dummy');
    useProject({
  ...
  it('should fail when the remote url is bad', async () => {
    const cwd = setupTmpDir();
    initGitRepo(cwd, { origin: 'bababooey' });
    createProjectLink(cwd, 'bad-remote-url');
    client.cwd = cwd;
    useUser();
    useTeams('team_dummy');
    useProject({
  ...
  it('should connect a repo to a project that is not already connected', async () => {
    const cwd = setupTmpDir();
    initGitRepo(cwd, { origin: 'https://github.com/user/repo' });
    createProjectLink(cwd, 'new-connection');
    client.cwd = cwd;
    useUser();
    useTeams('team_dummy');
    useProject({
    ...
    const project: Project = await client.fetch(`/v8/projects/new-connection`);
    expect(project.link).toMatchObject({
      type: 'github',
      repo: 'user/repo',
      repoId: 1010,
  ...
  it('should replace an old connection with a new one', async () => {
    const cwd = setupTmpDir();
    initGitRepo(cwd, { origin: 'https://github.com/user2/repo2' });
    createProjectLink(cwd, 'existing-connection');
    client.cwd = cwd;
    useUser();
    useTeams('team_dummy');
  ...
  it('should fail when it cannot find the repository', async () => {
    const cwd = setupTmpDir();
    initGitRepo(cwd, { origin: 'https://github.com/laksfj/asdgklsadkl' });
    createProjectLink(cwd, 'invalid-repo');
    client.cwd = cwd;
    useUser();
    useTeams('team_dummy');
    useProject({
  ...
  it('should connect the default option of multiple remotes', async () => {
    ...
    createProjectLink(cwd, 'multiple-remotes');
    client.cwd = cwd;
    useUser();
    useTeams('team_dummy');
    useProject({
  ...
  describe('connecting from a bare repository worktree', () => {
    let worktreePath: string;

    beforeAll(() => {
      const testDir = setupTmpDir('connect-worktree-test');
    ...
    it('should connect a project from within a worktree', async () => {
      createProjectLink(worktreePath, 'worktree-project');
      client.cwd = worktreePath;
      useUser();
      useTeams('team_dummy');
      useProject({
        ...defaultProject,
