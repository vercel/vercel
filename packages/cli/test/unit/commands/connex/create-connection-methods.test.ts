...
describe('connex create connection methods', () => {
  ...
  it('auto-picks the method the read resolved for a URL input', async () => {
    useConnexServices(client, {
      'mcp.notion.com': {
        ...notionServiceInfo,
        resolvedConnectionMethod: 'mcp',
      },
    });
    let postBody: any;
    client.scenario.post('/v1/connect/connectors/managed', (req, res) => {
    ...
    client.setArgv('connect', 'create', 'mcp.notion.com', '--name', 'from-url');
    const exitCodePromise = connect(client);

    // No target or method chooser: the URL already answered both.
    await expect(client.stderr).toOutput(
      'Connect to Notion with MCP (automatic registration)?'
    );
    client.stdin.write('\n');
  ...
  it('cancels when the resolved-method confirmation is declined', async () => {
    useConnexServices(client, {
      'mcp.notion.com': {
        ...notionServiceInfo,
        resolvedConnectionMethod: 'mcp',
    ...
    client.setArgv('connect', 'create', 'mcp.notion.com', '--name', 'nope');
    const exitCodePromise = connect(client);
  ...
  it('skips the resolved-method confirmation with --yes', async () => {
    useConnexServices(client, {
      'mcp.notion.com': {
        ...notionServiceInfo,
        resolvedConnectionMethod: 'mcp',
    ...
    mockConnexManagedCreate(client, { id: 'scl_url_yes', uid: 'notion/y' });
    ...
    expect(await connect(client)).toBe(0);
    expect(client.stderr.getFullOutput()).not.toContain('Connect to Notion');
  });

  it('falls back to the choosers when --target conflicts with the resolved method', async () => {
    useConnexServices(client, {
      'mcp.notion.com': {
        ...notionServiceInfo,
        resolvedConnectionMethod: 'mcp',
    ...
    // The resolved mcp method doesn't serve `api`, so the explicit target
    // wins and normal resolution takes over (two candidates → chooser).
    await expect(client.stderr).toOutput(
      'How do you want to connect to Notion?'
    );
    client.stdin.write('\n');
