...
async function prepareFixtures(session, binaryPath, tmpFixturesDir) {
  const spec = {
    'static-single-file': {
  ...
  for (const [fixtureName, fixtureContents] of Object.entries(spec)) {
    prepare(tmpFixturesDir, fixtureName, fixtureContents);
  }
}
...
async function prepare(tmpFixturesDir, fixtureName, fixtureContents) {
