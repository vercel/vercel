...
**Status:** Evals: **non-interactive** (prefer non-interactive flags for `vercel link`), **build** (run `vc build` on a minimal static site), **login-whoami** (verify CLI is logged in and `vercel whoami` returns the current user), **env/** (all env evals in one directory: **env/ls**, **env/add**, **env/pull**, **env/update**, **env/remove**). The runner discovers evals recursively in `evals/evals/` (any dir with PROMPT.md + EVAL.ts + package.json). If none are found it exits successfully; if any exist it runs `@vercel/agent-eval`. Add evals under `evals/evals/<name>/` (or nested, e.g. `evals/evals/env/add/`).
