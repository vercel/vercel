Go through the flow of **adding** an environment variable with `vercel env add` or `vc env add`, then **removing** it with `vercel env remove`, `vc env remove`, `vercel env rm`, or `vc env rm`.
