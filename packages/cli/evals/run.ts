...
function formatTimestampForResults(date = new Date()): string {
...
function writeLocalFixtureResults(evals: string[]): string {
  const timestamp = formatTimestampForResults();
  const runDir = join(__dirname, 'results', 'cli-local', 'fixture', timestamp);

  for (const evalName of evals) {
    const evalDir = join(runDir, evalName);
    const runOneDir = join(evalDir, 'run-1');
    mkdirSync(runOneDir, { recursive: true });

    writeJson(join(evalDir, 'summary.json'), {
      name: evalName,
      totalRuns: 1,
      passedRuns: 1,
      passRate: 1,
    ...
    writeJson(join(runOneDir, 'result.json'), {
      status: 'passed',
      error: null,
      duration: 0,
      metadata: {
        localFixture: true,
        note: 'Generated by packages/cli/evals/run.ts --local-fixture. This validates result ingestion shape only; it is not an agent execution.',
      },
    });
  }
...
function selectEvals(discovered: string[]): string[] {
  ...
  const base = requested.length > 0 ? requested : discovered;
...
async function main() {
  ...
  const isLocalFixture = args.includes('--local-fixture');

  const discoveredEvals = discoverEvals();
  const evals = selectEvals(discoveredEvals);
  if (evals.length === 0) {
    process.stdout.write(
      'No evals to run (evals/ has no matching fixtures with PROMPT.md + EVAL.ts + package.json).\n'
    );
    process.exit(0);
  }
  ...
  if (isLocalFixture) {
    const outputDir = writeLocalFixtureResults(evals);
    process.stdout.write(
      `\nWrote local fixture results for ${evals.length} eval(s): ${outputDir}\n`
    );
    process.stdout.write(
      'This validates the agent-eval results folder shape for local tooling and dashboard ingestion tests; it does not run an agent or Vercel CLI commands.\n'
    );
    process.exit(0);
  }
