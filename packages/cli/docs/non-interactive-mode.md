...
- Use **angle brackets** for placeholders in suggested commands: **`<slug>`**, **`<name>`**, **`<source>`**, **`<file>`**, **`<version-id>`**, **`<email>`**, etc.
...
- Prefer **`outputAgentError`** with a stable **`reason`** (e.g. `payment_required`, `rate_limited`, `slug_unavailable`, `timeout`, `domain_not_found`, `confirmation_required`) and a **plain `message`**—never chalk/`param()` in JSON.
