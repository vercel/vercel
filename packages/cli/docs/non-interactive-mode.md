...
- **`vercel api`:** API error responses are rendered as the **JSON error body** the API returned (e.g. `{"error": {"code": "forbidden", "action": "create", "resource": "project", ...}}`) on stdout with exit code `1` — in all modes, not only non-interactive. There is no `status`/`reason` wrapper — agents should read the `error` object directly (a 403 carries the denied `action`/`resource` so the caller can request the missing permission). The normal fetch flow (retries, SAML re-authentication) is unchanged; only the final error rendering differs. Errors without a parseable JSON body fall back to the prose message on stderr.
