...
- **`vercel api`:** In non-interactive mode, non-2xx responses are **passed through**: the raw API error body (e.g. `{"error": {"code": "forbidden", "action": "create", "resource": "project", ...}}`) is printed to stdout and the command exits `1`. There is no `status`/`reason` wrapper — agents should read the `error` object directly (a 403 carries the denied `action`/`resource` so the caller can request the missing permission). Implemented via **`returnErrorResponse: true`** on `client.fetch`, which also skips retries and SAML re-authentication.
