...
// Minimal FastAPIStaticMount for tests that only need urlPath.
function mount(urlPath: string): FastAPIStaticMount {
  return { urlPath, directory: '', fallback: null, frontend: false };
}
...
describe('fastapiShadowingRoutes', () => {
  ...
  it('ORs the (pre-escaped) shadow bodies into one group routed to the Lambda', () => {
    expect(
      fastapiShadowingRoutes(
        discovery({
          shadowRoutes: [
            { body: 'static/example\\.txt', methods: null },
            { body: 'items/(?:[0-9]+)', methods: null },
          ],
        }),
        'api/index'
      )
  ...
  it('splits bodies across routes when one src would exceed the 4096 cap', () => {
    ...
    const routes = fastapiShadowingRoutes(
      discovery({
        shadowRoutes: bodies.map(b => ({ body: b, methods: null })),
      }),
      'api/index'
    );
  ...
  it('drops a body too long for a src even alone, keeping the rest', () => {
    ...
    const routes = fastapiShadowingRoutes(
      discovery({
        shadowRoutes: ['before', 'x'.repeat(5000), 'after'].map(b => ({
          body: b,
          methods: null,
        })),
      }),
  ...
  it('accounts for the src wrapper at the cap boundary', () => {
    ...
    const fits = fastapiShadowingRoutes(
      discovery({ shadowRoutes: [{ body: 'a'.repeat(4085), methods: null }] }),
      'app'
    );
    ...
    expect(
      fastapiShadowingRoutes(
        discovery({
          shadowRoutes: [{ body: 'a'.repeat(4086), methods: null }],
        }),
        'app'
      )
    ).toEqual([]);
  ...
  it('emits one CDN route per distinct method set, preserving per-group ordering', () => {
    // Two API-route shadows (GET+HEAD) and one subtree shadow (null = all
    // methods) must not be merged into a single src. Each distinct method set
    // gets its own CDN route entry.
    const routes = fastapiShadowingRoutes(
      discovery({
        shadowRoutes: [
          { body: 'items', methods: ['GET', 'HEAD'] }, // API route shadow
          { body: 'other', methods: ['GET', 'HEAD'] }, // same group -> ORed together
          { body: 'api(?:/.*)?', methods: null }, // subtree -> separate group
        ],
      }),
      'api/index'
    );
    ...
    // The null-methods subtree route carries no `methods` restriction.
    const subtree = routes.find(r => r.src.includes('api'))!;
    expect(subtree.methods).toBeUndefined();
    // The GET+HEAD API routes are ORed into one src with those methods.
    const apiRoute = routes.find(r => r.src.includes('items'))!;
    expect(apiRoute.methods).toEqual(['GET', 'HEAD']);
    expect(apiRoute.src).toContain('items');
    expect(apiRoute.src).toContain('other');
  });
});
...
describe('fastapiFallbackRoutes', () => {
  ...
  it('excludes nested sibling mounts via a negative lookahead (root mount)', () => {
    const [route] = fastapiFallbackRoutes(
      discovery({
        collectedMounts: [mount('/'), mount('/assets'), mount('/api/docs')],
  ...
  it('escapes regex metacharacters in nested sibling sub-paths (guard)', () => {
    const [route] = fastapiFallbackRoutes(
      discovery({
        collectedMounts: [mount('/app'), mount('/app/a.b')],
  ...
  it('skips a fallback route whose src would exceed the route cap', () => {
    ...
    const collectedMounts = Array.from({ length: 400 }, (_, i) =>
      mount(`/static_assets_dir_${i}`)
    );
...
it('shadows both slash forms of a route declared with a trailing slash', () => {
  ...
  const [route] = fastapiShadowingRoutes(
    discovery({ shadowRoutes: [{ body: 'items', methods: null }] }),
    'api/index'
  );
  const matches = new RegExp(route.src);
...
it('shadows a colliding route under a sub-app but not its nested static sibling', () => {
  ...
  const [route] = fastapiShadowingRoutes(
    discovery({
      shadowRoutes: [
        { body: 'sub(?:/(?!(?:foo)(?:/|$)).*)?', methods: null },
        { body: 'sub/foo/bar', methods: null },
      ],
    }),
    'app'
  );
...
it('shadows a mount root but not the files under it', () => {
  ...
  const [route] = fastapiShadowingRoutes(
    discovery({ shadowRoutes: [{ body: 'static', methods: null }] }),
    'app'
  );
  const matches = new RegExp(route.src);
...
it('shadows both slash forms of an html=False directory index', () => {
  ...
  const [route] = fastapiShadowingRoutes(
    discovery({ shadowRoutes: [{ body: 'x/guide', methods: null }] }),
    'app'
  );
  const matches = new RegExp(route.src);
...
it('shadows a root-mounted sub-app subtree but not its nested CDN mounts', () => {
  ...
  const [route] = fastapiShadowingRoutes(
    discovery({
      shadowRoutes: [{ body: '(?!(?:static)(?:/|$)).*', methods: null }],
    }),
    'app'
  );
  const matches = new RegExp(route.src);
...
it('does not shadow a root mount inside a mounted sub-app', () => {
  ...
  const [route] = fastapiShadowingRoutes(
    discovery({
      shadowRoutes: [
        { body: 'sub', methods: null },
        { body: 'sub/hello', methods: null },
      ],
    }),
    'app'
  );
