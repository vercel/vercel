...
function generateDevShimSource(modulePath: string): string {
  // ASGI wrapper that serves PUBLIC_DIR first, then falls through to user's app
  // Intentionally avoids buffering unless the path resolves to an actual file
...
async def app(scope, receive, send):
    if scope.get('type') == 'http':
        ...
        try:
            # Ensure no directory traversal outside PUBLIC_DIR
            common = _p.commonpath([_p.realpath(full), _p.realpath(PUBLIC_DIR)])
            if common == _p.realpath(PUBLIC_DIR) and _p.isfile(full):
