...
const SHIM_MODULE_NAME = 'vc_asgi_static_dev_shim';
const SHIM_FILENAME = `${SHIM_MODULE_NAME}.py`;
const DEFAULT_PUBLIC_DIR = 'public';

function generateDevShimSource(modulePath: string): string {
  // ASGI wrapper: try static first. If it produces a <400 response, forward it.
  // Otherwise, fall through to the user app. Uses buffering to avoid partial writes.
  return `# Auto-generated by Vercel for vercel dev (Python)
# Serves static files from PUBLIC_DIR before delegating to the user app when not found.
...
USER_MODULE = ${JSON.stringify(modulePath)}
USER_APP = getattr(import_module(USER_MODULE), 'app')
PUBLIC_DIR = os.environ.get('VERCEL_DEV_PUBLIC_DIR', '${DEFAULT_PUBLIC_DIR}')

static_app = StaticFiles(directory=PUBLIC_DIR)

async def app(scope, receive, send):
    if scope.get('type') == 'http':
        # Buffer static response to inspect status before sending
        buffered = []
        status = {'code': None}

        async def send_buffered(message):
            if message.get('type') == 'http.response.start':
                status['code'] = int(message.get('status', 200))
            buffered.append(message)

        try:
            await static_app(scope, receive, send_buffered)
            code = status['code']
            ...
            # If static handling fails, fall through to user app
            pass
...
function maybeCreateDevStaticShim(
  workPath: string,
  modulePath: string,
  env: NodeJS.ProcessEnv
): string | null {
  const publicDir = join(workPath, DEFAULT_PUBLIC_DIR);
  if (!existsSync(publicDir)) return null;

  try {
    const shimPath = join(workPath, SHIM_FILENAME);
    const shimSource = generateDevShimSource(modulePath);
    writeFileSync(shimPath, shimSource, 'utf8');
    if (!env.VERCEL_DEV_PUBLIC_DIR)
      env.VERCEL_DEV_PUBLIC_DIR = DEFAULT_PUBLIC_DIR;
    debug(`Prepared Python dev static shim at ${shimPath}`);
    return SHIM_MODULE_NAME;
  } catch (err: any) {
  ...
  try {
    ...
    const childReady = new Promise<{ port: number; pid: number }>(
      (resolve, reject) => {
        ...
        // If a local `public/` directory exists, create a tiny ASGI shim
        // that serves static files first and falls back to the user's app.
        const devShimModule = maybeCreateDevStaticShim(
          workPath,
          modulePath,
          env
        );
