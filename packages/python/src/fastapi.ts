...
import { debug, FileBlob, FileFsRef, type Files } from '@vercel/build-utils';
import type { PyProjectToolVercelFastapiSection } from '@vercel/python-analysis';
...
export interface FastAPIStaticDiscovery {
  ...
  /**
   * Shadow routes for paths a higher-priority app route owns. Each carries the
   * regex body and the HTTP methods the originating route declares; the builder
   * emits one CDN route per distinct method set so the shadow only intercepts
   * the methods the app actually handles, leaving others to the CDN.
   */
  shadowRoutes: { body: string; methods: string[] | null }[];
}

/** Outcome of copying mounts to the CDN, for the builder to wire into routes. */
export interface FastAPICollectStaticResult {
  /** StaticFiles mounts collected to CDN, each with urlPath and source directory. */
  collectedMounts: FastAPIStaticMount[];
  cdnOutputDir: string;
...
export async function getFastAPIStaticDiscovery(
  ...
  workPath: string,
  suppressMiddlewareMounts: boolean
...
export async function runFastAPICollectStatic(
  ...
  entrypointAbs: string,
  variableName: string,
  suppressMiddlewareMounts: boolean = true
...
export interface FastAPICdnBundle {
  /** Glob patterns to exclude CDN-collected mount directories from the Lambda bundle. */
  excludePatterns: string[];
  /** Stub files that keep excluded mount directories present on the Lambda filesystem. */
  directoryStubs: Files;
  /**
   * Redirect routes to place before `handle: filesystem` in the CDN route table.
   */
  redirects: { src: string; dest: string; status: 307 }[];
}
...
export function fastapiShadowingRoutes(
  ...
  // Group shadow routes by method set: routes with the same methods can share
  // a chunked src; routes with different methods need separate CDN routes so
  // each shadow only intercepts the methods the app actually handles.
  const byMethods = new Map<string, string[]>();
  for (const sr of discovery.shadowRoutes) {
    // null means match all methods (sub-app subtree shadows)
    const key =
      sr.methods === null ? '' : JSON.stringify([...sr.methods].sort());
  ...
  for (const [methodsKey, bodies] of byMethods) {
    const methods: string[] | undefined =
      methodsKey === '' ? undefined : (JSON.parse(methodsKey) as string[]);
    const chunks: string[][] = [];
    let srcLen = 0;
...
export function fastapiFallbackRoutes(discovery: FastAPICollectStaticResult) {
  const mountPrefixes = discovery.collectedMounts.map(m =>
    trimTrailingSlashes(m.urlPath)
  );
...
/**
 * Compute all CDN bundle adjustments for a FastAPI build in one call:
 * - `excludePatterns`: passed to the glob ignore list so mount directory
 *   contents are not bundled into the Lambda.
 * - `directoryStubs`: empty placeholder files added back so FastAPI's
 *   `os.path.isdir(directory)` startup check still passes.
 *
 * Both are empty by default; set `tool.vercel.fastapi.static.exclude = true` to
 * populate them.
 */
export function getFastAPICdnBundle(
  collectedMounts: FastAPIStaticMount[],
  workPath: string,
  fastapiConfig: PyProjectToolVercelFastapiSection | undefined
): FastAPICdnBundle {
  if (fastapiConfig?.static?.exclude !== true)
    return { excludePatterns: [], directoryStubs: {}, redirects: [] };
  const excludePatterns: string[] = [];
  const directoryStubs: Files = {};
  const redirects: { src: string; dest: string; status: 307 }[] = [];
  for (const mount of collectedMounts) {
    const relDir = relative(workPath, mount.directory);
    if (!relDir || relDir.startsWith('..') || isAbsolute(relDir)) continue;
    const relDirUnix = relDir.split(sep).join('/');
    excludePatterns.push(`${relDirUnix}/**`);
    directoryStubs[`${relDirUnix}/.static-placeholder`] = new FileBlob({
      data: '',
    });
    // Keep any index.html / 404.html in the Lambda so FastAPI's startup
    // check for explicit fallback files passes.
    for (const fallbackFile of ['index.html', '404.html']) {
      const fsPath = join(mount.directory, fallbackFile);
      if (fs.existsSync(fsPath)) {
        directoryStubs[`${relDirUnix}/${fallbackFile}`] = new FileFsRef({
          fsPath,
        });
      }
    }
    // Redirect bare directory paths to their slash form so the CDN handles
    // the redirect before the fallback route can intercept the request.
    if (mount.frontend) {
      const urlPrefix = mount.urlPath.replace(/^\/+|\/+$/g, '');
      const addBareRedirects = (absDir: string) => {
        for (const entry of fs.readdirSync(absDir, { withFileTypes: true })) {
          if (!entry.isDirectory()) continue;
          const subAbs = join(absDir, entry.name);
          const subPath = relative(mount.directory, subAbs)
            .split(sep)
            .join('/');
