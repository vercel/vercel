...
it('should handle metadata files correctly', async () => {
  ...
  } = await runBuildLambda(path.join(__dirname, '../fixtures/00-metadata-files'));
  ...
  expect(output['opengraph-image']).toBeDefined();
  expect(output['opengraph-image'].type).toBe('Lambda');
