...
export const build: BuildV2 = async buildOptions => {
  ...
  const { traceState } = buildOptions;

  // If we have parent trace state being passed down then we restore it
  if (traceState) {
    initializeTraceState(traceState);
  }

  const buildSpan = new Span({ name: 'vc.next.build' });

  return buildSpan.traceAsyncFn(async () => {
    const {
      files,
      entrypoint,
      config = {},
    ...
    await buildSpan.traceChild('vc.builder.install').traceAsyncFn(async () => {
    ...
    const isCorrectLocaleAPIRoutes = semver.gte(
      nextVersion,
      'v11.0.2-canary.3'
    );
    ...
    if (isServerMode) {
      ...
      if (nowJsonPath) {
        ...
        if (
          Array.isArray(nowJsonData.routes) &&
          nowJsonData.routes.length > 0
        ) {
    ...
    await buildSpan.traceChild('vc.builder.build').traceAsyncFn(async () => {
      if (buildCommand) {
    ...
    if (isSharedLambdas) {
      ...
      for (let i = 0; i < dataRoutes.length; i++) {
        ...
        if (
          ...
          mergedDataRoutesLambdaRoutes.push(
            dynamicPageLambdaRoutesMap[pathname]
          );
        }
      }
