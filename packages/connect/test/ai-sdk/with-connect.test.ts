...
describe('withConnect', () => {
  ...
  it('resolves a token and runs execute with a bearer-authorized fetch', async () => {
    ...
    const tool = withConnect(
      { connectorId: CONNECTOR, subject: SUBJECT, scopes: ['read'] },
      {
        ...
        execute: async (_input, { fetch }) => {
          await fetch('https://api.linear.app/graphql', { method: 'POST' });
          return { ok: true };
        },
      }
    );
  ...
  it('exposes the raw token for SDKs above the fetch level', async () => {
