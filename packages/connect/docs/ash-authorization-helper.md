# Native Ash integration for `@vercel/connect`

**Status:** implemented (local), pending publish
**Owner:** @allenzhou
**Target release:** `@vercel/connect@0.1`
**Prerequisites:** Ash PR #316 merged (connection principal model); Ash alpha that exports `experimental-ash/connections` with `AuthorizationDefinition`, `ConnectionPrincipal`, `TokenResult`, `ConnectionAuthorizationChallenge`, `ConnectionAuthorizationRequiredError`, `ConnectionAuthorizationFailedError`, and `JsonValue`.

## Motivation

Every Ash app that uses Vercel Connect to broker MCP credentials today writes the same ~100 lines per connection. In `ash-demo-app/agent/connections/linear.ts` the only fields that actually differ from Notion are `url`, `description`, the `CONNECTOR_*` env name, and the `instructions` string. Everything else is boilerplate:

- read `connector` from env at module load and throw if missing,
- build the Vercel Connect `subject` from Ash's `ConnectionPrincipal`,
- catch `UserAuthorizationRequiredError` and rethrow as Ash's `ConnectionAuthorizationRequiredError`,
- catch `ConnectorInstallationRequiredError` and rethrow as Ash's `ConnectionAuthorizationFailedError`,
...
`connector` is the only required field. `principalType` defaults to `"user"`; authors set `"app"` only for app-scoped credentials. `connector` is a plain string — authors do their own env-var validation at module load (the typical pattern is a top-level `if (!value) throw ...`). Ash fills in `"Authorize <ConnectionName> in your browser to continue."` as the default consent-page prompt using the slot name it already derives from the filesystem path. Authors override via the optional `instructions` field only when they want non-default phrasing.

The helper imports Ash's authorization types directly via type-only imports from `experimental-ash/connections` and returns Ash's real `InteractiveAuthorizationDefinition<State>` / `NonInteractiveAuthorizationDefinition`. `experimental-ash` is declared as an **optional peer dependency** of `@vercel/connect`, and the helper lives at the `@vercel/connect/ash` subpath, so consumers that don't use Ash never load it.

### principalType → definition shape

Ash's `AuthorizationDefinition` is a discriminated union. The helper maps `principalType` onto it as follows:
...
| `principalType: "app"`            | `NonInteractiveAuthorizationDefinition` (`getToken` only) | `getToken` succeeds for a provisioned app connector; on failure the runtime throws `ConnectionAuthorizationFailedError` (`retryable: false`) because there is no user to consent. Ash v1 disallows interactive OAuth for app principals. |

Ash's `NonInteractiveAuthorizationDefinition` technically accepts `principalType: "user"` too (its "out-of-band OAuth" mode, where a user is authorized through a non-Connect flow and the agent just fetches the token). The helper does **not** expose that combination: with Vercel Connect specifically, a `"user"` token is always provisioned via Vercel Connect's own `startAuthorization`, so dropping the interactive methods would leave the runtime with no recovery path when `getToken` throws `UserAuthorizationRequiredError`. If that combination ever proves useful, it's an additive option later.

## Non-goals

- Not a general-purpose adapter for other agent frameworks. The helper imports Ash types directly; if a different framework adopts the same `AuthorizationDefinition` shape later, that's an additive subpath (`@vercel/connect/<framework>`), not this one.
- Not a rewrite of `getToken` / `startAuthorization`. Those remain the primitives; the helper is glue above them.
- Not a place to redefine framework types. The helper imports `AuthorizationDefinition`, `ConnectionPrincipal`, `TokenResult`, `ConnectionAuthorizationChallenge`, and the two error classes from `experimental-ash/connections` rather than mirroring them.
...
| Vercel Connect                                 | Ash                                                       | Translation                                                                                                                                                                |
| ---------------------------------------------- | --------------------------------------------------------- | -------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
...
| `UserAuthorizationRequiredError`               | `ConnectionAuthorizationRequiredError`                    | throw; Ash suspends turn                                                                                                                                                   |
...
| `startAuthorization` `callbackUrl` / `webhook` | `opts.webhook`                                            | always `callbackUrl: webhook` (Ash mints a browser-redirect URL with a branded landing page; `webhook:` would land the user on Vercel Connect's generic close-window page) |
| `verifier`                                     | `state.verifier`                                          | journaled; currently unused by Ash                                                                                                                                         |

## Proposed API

```ts
// packages/connect/src/ash.ts
 ...
 * index signature exists to satisfy Ash's `State extends JsonValue`
 * constraint.
 */
...
/**
 * Returns Ash's real `InteractiveAuthorizationDefinition` /
...
2. **`startAuthorization`** — calls Vercel Connect with `callbackUrl: webhook`. Ash's `webhook` parameter is semantically a browser-redirect target — the runtime mints it via `createWebhook({ respondWith: buildAuthorizationCompletePage() })` so the user lands on a branded "you can close this tab" page after consent. That maps to Vercel Connect's `callbackUrl:` semantics, which already accepts both `https://` (prod) and `http://localhost` (vercel dev), so one field covers both. We deliberately do **not** route `https://` URLs into Vercel Connect's `webhook:` (server-POST) field even though that would be more robust to the user closing the consent tab right after IdP callback: that mode shows the user Vercel Connect's generic "close this window" page instead of Ash's branded landing page, and the helper would need to grow protocol-aware logic that diverges from the simple "Ash mints one URL, Vercel Connect redirects there" mental model. Revisit if tab-close timeouts become a real problem in production. Returns `{ challenge: { url, instructions? }, state: { verifier } }`. `instructions` is only set when the author provides one — otherwise Ash fills in the default in its runtime step (see `withDefaultAuthorizationInstructions`).
...
When Vercel Connect (or the underlying IdP) surfaces an error Ash doesn't know how to classify, the `onError` hook lets you rewrite it without copy-pasting the whole try/catch. Because the helper imports Ash's real error classes via the peer dep, you can construct them directly in your hook:

```ts
import { connect } from "@vercel/connect/ash";
import { ConnectionAuthorizationFailedError } from "experimental-ash/connections";
...
Ash owns `ConnectionAuthorizationRequiredError` / `ConnectionAuthorizationFailedError` in `experimental-ash/connections`. The first iteration of this plan considered three approaches:

**Option A: branded duck-typing.** Tag errors with `Symbol.for("ash.error.connection-authorization")` and have Ash discriminate on the brand instead of `instanceof`. Vercel Connect throws plain errors with the brand attached and never imports Ash.

**Option B: optional Ash subpath.** `@vercel/connect/ash` imports Ash's error classes directly via a type-only import + an optional peer dependency. Real `instanceof` works at the call site; consumers that don't use Ash never load the subpath.
...
**Decision: Option B.** The helper lives at the `@vercel/connect/ash` subpath. `experimental-ash` is declared as an optional peer dependency in `@vercel/connect/package.json`; importing the subpath requires Ash to be installed in the consumer project, but the rest of `@vercel/connect` works without it.

Why we shifted off Option A:

- **Type drift was already a problem.** While the helper structurally mirrored Ash's `AuthorizationDefinition`, every Ash-side change (renaming `instructions`, narrowing `principal.attributes`, adding `reason` codes) silently went out of sync until something broke at runtime. A type-only import from Ash's actual definitions removes the drift surface.
- **The branding work belongs in Ash regardless.** Ash already discriminates on `err.name`, not `instanceof` (see `isConnectionAuthorizationRequiredError`), specifically because of the bundler-duplication risk. That solves the cross-realm problem without needing `Symbol.for`. With `err.name` discrimination already shipped, Option A's primary benefit collapses.
- **Optional peer deps are well-understood.** React, `@types/*` packages, and many lint plugins use this pattern. Subpath exports + `peerDependenciesMeta.optional` keep non-Ash consumers honest: if you only use the low-level Vercel Connect SDK from `@vercel/connect`, nothing pulls Ash in.
- **Inverted layering is a non-issue here.** `@vercel/connect` is a Vercel-platform SDK; `experimental-ash` is a separately-published framework. Neither owns the other. The subpath split makes the dependency _opt-in per import_, which is materially different from a hard dep.

Implementation summary:

- `peerDependencies: { "experimental-ash": ">=0.3.0-alpha.19" }` + `peerDependenciesMeta.experimental-ash.optional: true`.
- Subpath export `./ash` → `./dist/ash.js` / `./dist/ash.d.ts`.
- `src/ash.ts` imports types and the two error classes from `experimental-ash/connections` and throws Ash's actual error instances. `src/index.ts` does **not** re-export anything from `ash.ts`.
...
packages/connect/src/
  ...
  ash.ts            # helper, exposed at the `@vercel/connect/ash` subpath
...
```jsonc
{
  "exports": {
    ...
    "./ash": { "types": "./dist/ash.d.ts", "default": "./dist/ash.js" },
  },
  "peerDependencies": {
    "experimental-ash": ">=0.3.0-alpha.19",
  },
  "peerDependenciesMeta": {
    "experimental-ash": { "optional": true },
  },
  "devDependencies": {
    "experimental-ash": "<pinned>",
    "typescript": "^5",
  },
}
```

The devDep on `experimental-ash` exists so the Vercel Connect repo can typecheck `src/ash.ts` locally; it's not part of the published artifact. While the principal-model branch is unreleased, a pnpm workspace override at the Vercel Connect monorepo root points `experimental-ash` at the local Ash checkout.
...
`agent/lib/connect.ts` is deleted — the env-var check is inlined at the top of each connection module (each connection knows its own env var name, so the inline form reads cleaner than a shared helper). The helper never needs the connection name: Ash's orchestrator attaches the slot name (derived from the filesystem path) to re-thrown errors and to the default `instructions` text.

`tsconfig.json` gains an explicit `"types": ["node"]` — pulling in the type-only chain through `@vercel/connect/ash` → `experimental-ash` confused tsgo's auto-`@types/*` walk in our setup, and an explicit list pins it deterministically.
...
2. **Vercel Connect SDK change — [DONE locally, pending publish].**
   ...
   - Keep `src/index.ts` limited to the low-level Vercel Connect SDK (no Ash exports leak to the root entrypoint).
   - **Pending:** unit tests (`packages/connect/src/ash.test.ts`):
     ...
     - `startAuthorization` wraps Vercel Connect response; `callbackUrl` set to the Ash-minted webhook URL.
...
4. **Demo app migration — [DONE].**
   ...
   - Rerun `scripts/smoke-linear.mjs` and `scripts/smoke-oauth.mjs` after the Ash alpha publishes.

5. **Docs.** Update `connect/README.md`, `connect/skills/vercel-connect/SKILL.md`, and reference the helper from `ash/research/connections/connect-sts-demo-integration.md`.

## Testing strategy

- **Unit (Vercel Connect):** step 2 above, all network mocked.
- **Unit (Ash):** `withDefaultAuthorizationInstructions` tests cover the default-instructions fill-in (already in `src/execution/authorization-challenge-defaults.test.ts`).
- **Integration (demo app):** existing smoke scripts exercise the full Ash runtime + Vercel Connect + IdP. Regression gate.
- **No e2e needed** — the helper is pure glue; Vercel Connect primitives own network behavior and already have coverage.

## Open questions

1. **Forward `issuer` for user principals?** Ash carries `principal.issuer`; Vercel Connect ignores it today. Leave a TODO pointing at the follow-up.
...
5. **Is `state: { verifier }` useful?** Ash journals it but does not read it. Cheap to keep for debuggability; propose: keep.
...
- **Blast radius:** additive on the Vercel Connect side. `src/ash.ts` is reachable only via the new `@vercel/connect/ash` subpath; existing imports of `@vercel/connect` are untouched.
- **Rollback:** revert the Vercel Connect PR; existing explicit-shape connection code still compiles. The `ash.ts` module is dist-only (no source-level cross-cutting), so the rollback is a single file delete plus a `package.json` revert.
- **Ash coupling:** scoped to consumers that import `@vercel/connect/ash`. The optional peer dep keeps non-Ash consumers free of `experimental-ash` in their lockfile. If Ash ever ships a breaking change to `AuthorizationDefinition`, the helper's typecheck catches it at compile time (vs the previous structural mirror, which silently went out of sync).

## Appendix A — Why not inline this as an Ash helper?

Alternative: put `defineConnectMcpClientConnection` in `experimental-ash/connections/connect` and have Ash depend on `@vercel/connect`.
...
- Ash would take a hard dep on `@vercel/connect` — fine today, bad if we ever split.
- Non-Ash frameworks could not use the helper.

**For:**

- Intellisense discoverability for Ash authors.
...
Ash caches `TokenResult` in virtual context per step (`ash/runtime/connections/authorization-tokens.ts`). Vercel Connect caches `ConnectTokenResponse` in-process per `(connector, params)` tuple until `expiresAt - validityBufferMs`. The helper introduces no additional cache:

- First call in a step: Ash cache miss → helper → Vercel Connect SDK → Vercel Connect server.
- Subsequent calls in the same step: Ash cache hit; helper never runs.
- Next step, same turn: Ash cache miss (virtual wiped) → helper → Vercel Connect SDK (cache hit) → no HTTP.
- Turn N+1: same path. Vercel Connect SDK cache persists across turns.

Fast path: Ash virtual cache → Vercel Connect in-memory cache → Vercel Connect server. No correctness implications; just a note for anyone sizing throughput.
