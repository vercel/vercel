...
export function getStandaloneServerRoutes(
  ...
  // V1 service routing is generated by fs-detectors and merged into a shared
  // top-level table. V2 services omit `type` and use isolated route tables, so
  // they need a catch-all to reach their internal Lambda.
  ...
  return [
    ...
    {
      src: '/(.*)',
      dest: getInternalServiceFunctionPath(service.name),
    },
  ];
}
