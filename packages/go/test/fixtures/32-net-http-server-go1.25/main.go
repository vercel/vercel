...
func main() {
	...
	mux.Handle("GET /static/", http.StripPrefix("/static/", http.FileServer(http.FS(assetFS))))

	mux.HandleFunc("GET /{$}", func(w http.ResponseWriter, r *http.Request) {
		w.Header().Set("Content-Type", "text/html; charset=utf-8")
		_, _ = w.Write([]byte("<html><body>framework:net-http version:" + runtime.Version() + " randomness:RANDOMNESS_PLACEHOLDER</body></html>"))
	})
	...
	mux.HandleFunc("GET /api/items/{id}", func(w http.ResponseWriter, r *http.Request) {
		writeJSON(w, http.StatusOK, ItemResponse{
			Item: DataItem{
				ID:    1,
				Name:  "Sample Item " + r.PathValue("id"),
				Value: 100,
			},
			Framework: "net-http",
			Version:   runtime.Version(),
