...
export interface RoutingResult {
  dest_path: string;
  headers: Record<string, string | undefined>;
  important_headers: Record<string, string | undefined>;
  status: number;
...
function apply_overriden_middleware_headers(
  req_headers: Record<string, string | undefined>,
  resp_headers: Record<string, string | undefined>
) {
  const overriddenHeaders = resp_headers['x-middleware-override-headers'];
...
export function get_resolver({
  ...
  function generate_result_cache_key({
    ...
    const getKeyPart = (keys: string[], values: Record<string, string>) => {
      let keyPart = '';
  ...
  return async function resolve_routes({
    ...
    const request_path_key = `${request_method}${host}${deployment_id}${wildcard}${request_path}${error_status}`;
      ...
      for (const route of routes) {
        ...
        if (
          ...
          if (middleware_result.headers) {
            ...
            const redirect = middleware_result.headers['x-middleware-redirect'];
            ...
            const rewrite = middleware_result.headers['x-middleware-rewrite'];
            ...
            for (const key of Object.keys(middleware_result.headers)) {
              routing_result.headers[key] = middleware_result.headers[key];
            }
          }
