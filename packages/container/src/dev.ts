...
export async function startDevServer(
  ...
  return startDevServerInternal(options);
}

/**
 * Start a local container from a source generated by another first-party
 * framework adapter.
 */
export async function startDevServerWithContainerSource(
  options: StartDevServerOptions,
  source: ContainerBuildSource
): Promise<StartDevServerResult> {
  return startDevServerInternal(options, source);
}

async function startDevServerInternal(
  options: StartDevServerOptions,
  source?: ContainerBuildSource
): Promise<StartDevServerResult> {
  ...
  const startPromise = startContainer(options, reuseKey, source).finally(() => {
  ...
  return withSpan(
    ...
    async span => {
      ...
      const image = await withSpan(span, 'container.dev.resolve_image', {}, s =>
        resolveDevImage(options, out, s, source)
      );
