...
module.exports = class Alias extends Now {
  ...
  async set(deployment, alias, domains) {
    ...
    if (aliasDepl) {
      ...
      if (aliasedDeployment && aliasedDeployment.scale.current > depl.scale.current && aliasedDeployment.scale.current >= aliasedDeployment.scale.min && aliasedDeployment.scale.current <= aliasedDeployment.scale.max) {
        ...
        const scaleTime = Date.now();
        if (depl.scale.max < 1) {
          if (this._debug) {
            console.log("Updating max scale to 1 so that deployment may be unfrozen.");
          }
          const ress = await this.setScale(depl.uid, {min: depl.scale.min, max: Math.max(aliasedDeployment.scale.max, 1) });
        }
