...
module.exports = class Alias extends Now {
  ...
  async rm(_alias) {
    return this.retry(async bail => {
      const res = await this._fetch(`/now/aliases/${_alias.uid}`, { method: 'DELETE' });
  ...
  async set(deployment, alias, domains) {
    ...
    if (aliasDepl && aliasDepl.rules) {
      if (isTTY) {
        try {
          const msg = `> Path alias excists with ${aliasDepl.rules.length} rule${aliasDepl.rules.length >
            1
            ? 's'
            : ''}.\n` +
        ...
        console.log(
          `Overwriting path alias with ${aliasDepl.rules.length} rule${aliasDepl.rules.length > 1
            ? 's'
    ...
    let shouldMatchScale = false;
    if (aliasDepl) {
      const aliasedDeployment = await this.findDeployment(aliasDepl.deploymentId);
      if (aliasedDeployment && aliasedDeployment.scale.current > depl.scale.current) {
        if (isTTY) {
          try {
            const msg = `> Current aliased deployment is scaled to ${aliasedDeployment.scale.current} while the new deployment is only scaled to ${depl.scale.current}` +
              `> Would you like to scale it before updating alias?\n`;

            const confirmation = await promptBool(msg);

            if (confirmation) {
              shouldMatchScale = true;
            } else {
              console.log('> Updating alias without scaling it first');
            }
          } catch (err) {
            console.log(err);
          }
      ...
      if (shouldMatchScale) {
        ...
        if (depl.scale.current < 1) {
          console.log(`> Deployment ${deopl.url} is frozen, unfreezing...`);
          await this.unfreeze(depl);
          console.log(`> Deploymet is now unfrozen, scaling it to match current instance count`);
        }
        // Scale it to current limit
        console.log(`> Scaling deployment to match current scale.`);
        const updatedScale = await this.setScale(depl.uid, {
          min: aliasedDeployment.scale.current,
          max: aliasedDeployment.scale.current
        });
        // And then make min and max to match..
        console.log(`> Updating scaling rules for deployment.`);
        await scaleInfo(now, depl.url)

        const { scale } = aliasedDeployment;
        const updatedScale2 = await this.setScale(depl.uid, {
    ...
    if (created) {
      ...
      const output = `${chalk.cyan('> Success!')} Alias created ${chalk.dim(
        `(${uid})`
      )}:\n${chalk.bold(chalk.underline(pretty))} now points to ${chalk.bold(
        `https://${depl.url}`
      )} ${chalk.dim(`(${depl.uid})`)}`;
      if (isTTY && clipboard) {
        let append;
  ...
  async maybeSetUpDomain(alias, domains) {
    const gracefulExit = () => {
      this.close()
      domains.close()
      process.exit()
    ...
    if (!/\.now\.sh$/.test(alias)) {
      ...
      const pricePromise = domains.price(alias)
      const canBePurchased = await domains.status(alias)
      const aliasParam = param(alias)

      stopSpinner()

      if (canBePurchased) {
        const price = await pricePromise
        info(`The domain ${aliasParam} is not registered and ${chalk.bold('available for purchase')}.`)
        ...
        if (!domainInfo.verified) {
          ...
          tld = param(`.${tld}`)
          error('The nameservers are pending propagation. Please try again shortly')
      ...
      if (!usingZeitWorld && !skipDNSVerification) {
        ...
        console.log(
          `${chalk.cyan('> Success!')} Domain ${chalk.bold(chalk.underline(domain))} ${chalk.dim(
            `(${uid})`
          )} added`
        );
      }
        ...
        if (targets.indexOf(ip) === -1) {
          const err = new Error(
            `The domain ${domain} has an A record ${chalk.bold(
              ip
            )} that doesn't resolve to ${chalk.bold(chalk.underline('alias.zeit.co'))}.\n> ` +
