  ...
  async set(deployment, alias, domains) {
    ...
    if (aliasDepl && aliasDepl.rules) {
      if (isTTY) {
        ...
        console.log(
          `Overwriting path alias with ${aliasDepl.rules.length} rule${aliasDepl.rules.length >
            1
            ? 's'
    ...
    if (created) {
      const output = `${chalk.cyan(
        '> Success!'
      )} ${alias} now points to ${chalk.bold(depl.url)}! ${chalk.grey('[' + ms(Date.now() - aliasTime) + ']')}`;
      if (isTTY && clipboard) {
        let append;
        try {
          await copy(depl.url);
  ...
  async maybeSetUpDomain(alias, domains) {
    ...
    if (!/\.now\.sh$/.test(alias)) {
      ...
      if (canBePurchased) {
        ...
        info(
          `The domain ${aliasParam} is not registered and ${chalk.bold(
