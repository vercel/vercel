  ...
  async set(deployment, alias, domains) {
    ...
    if (aliasDepl && aliasDepl.rules) {
      if (isTTY) {
        try {
          const msg =
            `> Path alias excists with ${aliasDepl.rules.length} rule${aliasDepl.rules.length > 1 ? 's' : ''}.\n` +
  ...
  async maybeSetUpDomain(alias, domains) {
    ...
    if (!/\.now\.sh$/.test(alias)) {
      ...
      if (canBePurchased) {
        ...
        info(
          `The domain ${aliasParam} is not registered and ${chalk.bold('available for purchase')}.`
        );
        const confirmation = await promptBool(`Buy now for ${chalk.bold(`$${price}`)}?`);
        eraseLines(1);
        if (!confirmation) {
          info('Aborted');
          gracefulExit();
