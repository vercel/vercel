...
module.exports = function ({
  ...
  return new Promise((resolve, reject) => {
    ...
    if (mask === 'cc') {
      ...
      stdout.write(value)
      stdout.write(ansiEscapes.cursorBackward(19))
    } else {
      value = initialValue
    ...
    function onData(buffer) {
      ...
      if (data === ESCAPES.LEFT) {
        ...
        if (mask === 'cc' && value.length > Math.abs(caretOffset)) {
        ...
        if (mask === 'cc') {
          if (/\d/.test(data) && value.length === 19 && caretOffset !== 0) {
            if (value[value.length + caretOffset + 1] === ' ') {
      ...
      if (mask === 'cc') {
        ...
        value = value.replace(/#/g, chalk.gray('#'))
      }
