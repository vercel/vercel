...
function barify(cur, tot) {
  return '[' + range(0, cur).map(() => '=').join('') + range(cur, tot).map(() => '-').join('') + ']';
}

module.exports = async function info(now, url = argv._[1]) {
  ...
  linelog(`> Scaling to ${chalk.bold(String(targetReplicaCount) + (targetReplicaCount !== 1 ? ' instances' : ' instance'))}: ` + barify(barcurr, end));
    ...
    async () => {
      ...
      if (barcurr !== res.length) {
        ...
        if (barcurr === targetReplicaCount) {
          linelog.clear();
          linelog(`> Scaled to ${chalk.bold(String(targetReplicaCount) + (targetReplicaCount !== 1 ? ' instances' : ' instance'))}: ${chalk.gray('[' + ms(Date.now() - startTime) + ']')}\n`);
          return res;
        }
      }
