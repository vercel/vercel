  ...
  for (const task of turboJson.tasks) {
    ...
    if (task.dependencies.length > 0) {
      for (const dependency of task.dependencies) {
        ...
        // pnpm 8 fails to install dependencies with @ in the URL; scoped names
        // need both %40 and %2F (e.g. @vercel/vc-native-* → %40vercel%2F...).
        const escapedName = name.replace('@', '%40').replace('/', '%2F');
        ...
        if (
          ...
          name in packageObj.optionalDependencies
        ) {
          packageObj.optionalDependencies[name] = tarballUrl;
        }
      }
    }

    // Native optional deps are not in turbo's graph (they are generated by
    // release-binary). For preview tarballs we must rewrite them to preview
    // URLs so `npx https://.../vercel.tgz` does not fetch a stale registry
    // binary. npm treats optional dep fetch failures as non-fatal, so if the
    // preview native tarballs don't exist the trampoline safely falls back to JS.
    if (packageObj.optionalDependencies) {
      for (const name of Object.keys(packageObj.optionalDependencies)) {
        if (name.startsWith('@vercel/vc-native-')) {
          const escapedName = name.replace('@', '%40').replace('/', '%2F');
          const tarballUrl = `https://${process.env.VERCEL_URL}/tarballs/${escapedName}.tgz`;
          packageObj.optionalDependencies[name] = tarballUrl;
        }
