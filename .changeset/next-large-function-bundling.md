...
When `NEXT_EXPERIMENTAL_LARGE_FUNCTION_BUNDLING` is set, any route whose own uncompressed size does not fit the default per-runtime packing budget (the size limit `DEFAULT_MAX_UNCOMPRESSED_LAMBDA_SIZE` minus the reserved headroom `LAMBDA_RESERVED_UNCOMPRESSED_SIZE` — e.g. 250 MiB − 25 MiB on Node) is emitted as its own individual function, measured against a higher 5 GiB ceiling (`DEFAULT_MAX_UNCOMPRESSED_LARGE_LAMBDA_SIZE`) instead of the default limit. Such routes are never bundled — neither with normally-sized routes nor with each other. The default bundling pool is unchanged.
