...
Also detect the Git repository from subdirectories, worktrees, and submodules. Linking from `apps/web` previously found no `.git` and silently skipped the question, so the repository was never offered for connection. Worktrees and submodules were skipped for the same reason, since `.git` is a file rather than a directory there.
