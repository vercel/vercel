...
builds, stateless runtime defaults, Services command overrides, and local
`vercel dev` support without committing a Dockerfile. Apps using the
`vercel/laravel` Composer adapter also get private `queue/v2beta` consumers that
execute jobs through Laravel's native worker.
