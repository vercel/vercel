...
export default async function generateCertForDeploy(output: Output, client: Client, contextName: string, deployURL: string) {
  ...
  const cert = await createCertForCns(client, [domain, `*.${domain}`], contextName);
  cancelCertWait();
