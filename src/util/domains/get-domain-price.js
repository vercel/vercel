...
export default async function getDomainPrice(now, name, coupon) {
  if (coupon) {
  ...
  try {
    const payload = await now.fetch(`/v3/domains/price?${stringify({ name })}`);
    return coupon ? { price: 0, period: payload.price } : payload;
  } catch (error) {
