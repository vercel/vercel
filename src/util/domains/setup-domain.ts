...
export default async function setupDomain(output: Output, client: Client, alias: string, contextName: string) {
  ...
  const purchasedDomain = (await maybeGetDomainByName(client, contextName, domain) as Domain);
