...
async function getCertsToDelete(
  ...
  if (cert instanceof ERRORS.CertNotFound || (cert && cert.uid !== id)) {
    let certs = await getCertsForDomain(output, client, contextName, id);
    if (certs instanceof ERRORS.CertsPermissionDenied) {
      return certs;
    }
    certs = filterMatchingCert(id)(certs);
    return certs;
  }
  return [cert];
}

function filterMatchingCert(id: string) {
  return (certs: Cert[]) =>
    certs.filter(cert => cert.cns.some(cn => cn === id));
}
