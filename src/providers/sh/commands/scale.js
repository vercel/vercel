...
module.exports = async function main(ctx: CLIContext): Promise<number> {
  ...
  const verifyResult = await waitVerifyDeploymentScale(output, now, deployment.uid, updatedDeployment.scale)
  if (verifyResult instanceof VerifyScaleTimeout) {
    output.error(`Instance verification timed out (${ms(verifyResult.meta.timeout)})`, 'verification-timeout')
    now.close()
    return 1
  }
